# SpeechBrain · Deep Dive

> OSS PyTorch toolkit · STT · TTS · speaker ID · enhancement · diarization
> Category: Speech Toolkit · License: Apache-2.0 · Port: none

## 1. Overview + when-to-use

OSS PyTorch toolkit · STT · TTS · speaker ID · enhancement · diarization

### When to use SpeechBrain

| Use SpeechBrain when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> speechbrain["SpeechBrain"]
    speechbrain --> Out["Audio / Text out"]
    speechbrain --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool speechbrain
```
...
| §91 layer | Default | With SpeechBrain |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | SpeechBrain plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **SpeechBrain** |

### Wiring into LangGraph node

```python
# ai-agents/speechbrain/deep/backend/adapter.py (operator-implemented)
...
- Tool homepage: (search) SpeechBrain
