# RAG (Chroma) · Deep Dive

> Multi-tenant retrieval. Embedding · vector search · per-tenant scoping. Per §79.
> License: Apache-2.0 · Port: 8001 (Chroma)
...
### When to use RAG (Chroma)

| Use RAG (Chroma) when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> rag["RAG (Chroma)"]
    rag --> Audit["§38.3 audit row"]
    rag --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool rag
```

## Integration with §91 stack

### How RAG (Chroma) composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With RAG (Chroma) |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does RAG (Chroma) replace?) |
...
```python
# ai-agents/rag/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on 8001 (Chroma) | `docker compose down` or change port via env |
