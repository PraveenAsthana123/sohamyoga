...
> OSS customer engagement · email/SMS/push · journey builder · alternative to Customer.io
> Category: Marketing Automation · License: MIT · Port: 3000

## 1. Overview + when-to-use

OSS customer engagement · email/SMS/push · journey builder · alternative to Customer.io

### When to use Dittofeed

| Use Dittofeed when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> dittofeed["Dittofeed"]
    dittofeed --> Out["Output (file / API / DB / queue)"]
    dittofeed --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool dittofeed
```
...
| §91 layer | Default | With Dittofeed |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Dittofeed plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Dittofeed** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/dittofeed/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check SMTP + DB |
...
- Tool homepage: search "Dittofeed"
