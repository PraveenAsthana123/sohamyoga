# UI-TARS-7B · Deep Dive

> ByteDance vision-language model · OCR-free GUI · needs 16GB GPU.
> License: Apache-2.0 (model) · Port: 8002 (vLLM)
...
### When to use UI-TARS-7B

| Use UI-TARS-7B when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> ui-tars["UI-TARS-7B"]
    ui-tars --> Audit["§38.3 audit row"]
    ui-tars --> Output["downstream result"]
```
...
```bash
git lfs install && git clone https://huggingface.co/bytedance-research/UI-TARS-7B-DPO ~/models/ui-tars && pip install vllm
```
...
### How UI-TARS-7B composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With UI-TARS-7B |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does UI-TARS-7B replace?) |
...
```python
# ai-agents/ui-tars/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on 8002 (vLLM) | `docker compose down` or change port via env |
