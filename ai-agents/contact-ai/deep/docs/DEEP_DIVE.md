# Contact AI · Deep Dive

> CRM-style contact management AI · enrichment · entity resolution · routing · scoring · segmentation. DISTINCT from contact center (conversational) — this is contact management (CRM).

## 1. Overview + when-to-use

CRM-style contact management AI · enrichment · entity resolution · routing · scoring · segmentation. DISTINCT from contact center (conversational) — this is contact management (CRM).

### When to use Contact AI

| Use Contact AI when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Goal["Goal · cohort · prompt"] --> contact-ai["Contact AI"]
    contact-ai --> Out["Output artifact (image / contact record / etc.)"]
    contact-ai --> Audit["§38.3 audit row"]
    contact-ai --> Vector["§87.4 vector ingest"]
```

## 3. Install

```bash
./scripts/setup_ai_agent_stack.sh --tool contact-ai
```
