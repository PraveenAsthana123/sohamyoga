...
> Self-hosted form builder · OSS alternative to Typeform · MongoDB-backed
> Category: Survey/Forms · License: AGPL-3 · Port: 5000

## 1. Overview + when-to-use

Self-hosted form builder · OSS alternative to Typeform · MongoDB-backed

### When to use OhMyForm

| Use OhMyForm when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> ohmyform["OhMyForm"]
    ohmyform --> Out["Output (file / API / DB / queue)"]
    ohmyform --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool ohmyform
```
...
| §91 layer | Default | With OhMyForm |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | OhMyForm plugs in as a tool/connector |
| Side-effect channel | (per use case) | **OhMyForm** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/ohmyform/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check MongoDB |
