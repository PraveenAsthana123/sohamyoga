...
> Streaming STT · real-time + batch · Nova-3 model · diarization
...
| Use Deepgram when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    deepgram --> Out["Audio / Text out"]
    deepgram --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool deepgram
```
...
| §91 layer | Default | With Deepgram |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Deepgram plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Deepgram** |

### Wiring into LangGraph node

```python
# ai-agents/deepgram/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set DEEPGRAM_API_KEY · refresh from dashboard |
