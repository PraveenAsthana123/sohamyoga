# GSD (Goal-Spec-Driven) · Deep Dive

> Goal → Spec → Code methodology · uses LLM to convert high-level goals into precise specs before code · OSS
...
### When to use GSD (Goal-Spec-Driven)

| Use GSD (Goal-Spec-Driven) when... | Use alternative when... |
|---|---|
...
```bash
./scripts/setup_ai_agent_stack.sh --tool gsd
```
...
| §91 layer | Default | With GSD (Goal-Spec-Driven) |
|---|---|---|
...
| Methodology | (manual) | **GSD (Goal-Spec-Driven)** drives the dev loop |
...
- Tool homepage: search "GSD (Goal-Spec-Driven)"
