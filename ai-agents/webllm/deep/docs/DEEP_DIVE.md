...
> Browser-native LLM via WebGPU. Zero server cost · zero data egress. Llama-3.1-8B / Phi-3.5-mini.
> License: Apache-2.0 · Port: n/a (browser)
...
| Use WebLLM when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> webllm["WebLLM"]
    webllm --> Audit["§38.3 audit row"]
    webllm --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool webllm
```

## Integration with §91 stack

### How WebLLM composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With WebLLM |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does WebLLM replace?) |
...
```python
# ai-agents/webllm/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on n/a (browser) | `docker compose down` or change port via env |
