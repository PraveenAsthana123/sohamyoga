# LimeSurvey · Deep Dive

> OSS survey tool · advanced logic · 80+ question types · multi-language · LDAP
> Category: Survey/Forms · License: GPL-2 · Port: 8080

## 1. Overview + when-to-use

OSS survey tool · advanced logic · 80+ question types · multi-language · LDAP

### When to use LimeSurvey

| Use LimeSurvey when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> limesurvey["LimeSurvey"]
    limesurvey --> Out["Output (file / API / DB / queue)"]
    limesurvey --> Audit["§38.3 audit row"]
```
...
| §91 layer | Default | With LimeSurvey |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | LimeSurvey plugs in as a tool/connector |
| Side-effect channel | (per use case) | **LimeSurvey** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/limesurvey/deep/backend/adapter.py (operator-implemented)
...
- Tool homepage: search "LimeSurvey"
