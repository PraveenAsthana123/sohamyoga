...
> Production SDXL/Flux UI · multi-user · node graph workflow · advanced compositing
> Category: Image Generation · License: Apache-2.0 · Port: 9090
...
| Use InvokeAI when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> invokeai["InvokeAI"]
    invokeai --> Out["Output (file / API / DB / queue)"]
    invokeai --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool invokeai
```
...
| §91 layer | Default | With InvokeAI |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | InvokeAI plugs in as a tool/connector |
| Side-effect channel | (per use case) | **InvokeAI** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/invokeai/deep/backend/adapter.py (operator-implemented)
