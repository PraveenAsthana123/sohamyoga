# OmniParser · Deep Dive

> Microsoft GUI screenshot → structured DOM. Lightweight vision.
> License: MIT · Port: 8003
...
| Use OmniParser when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> omniparser["OmniParser"]
    omniparser --> Audit["§38.3 audit row"]
    omniparser --> Output["downstream result"]
```
...
```bash
git clone https://github.com/microsoft/OmniParser.git && cd OmniParser && pip install -r requirements.txt
```

Or use the universal installer (preferred):
```bash
./scripts/setup_ai_agent_stack.sh --tool omniparser
```

## Integration with §91 stack

### How OmniParser composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With OmniParser |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does OmniParser replace?) |
...
```python
# ai-agents/omniparser/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on 8003 | `docker compose down` or change port via env |
