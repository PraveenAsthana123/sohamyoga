...
> OSS Node.js newsletter platform · campaign + list management · Amazon SES integration
> Category: Email/Newsletter · License: GPL-3 · Port: 3000

## 1. Overview + when-to-use

OSS Node.js newsletter platform · campaign + list management · Amazon SES integration

### When to use Mailtrain

| Use Mailtrain when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> mailtrain["Mailtrain"]
    mailtrain --> Out["Output (file / API / DB / queue)"]
    mailtrain --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool mailtrain
```
...
| §91 layer | Default | With Mailtrain |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Mailtrain plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Mailtrain** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/mailtrain/deep/backend/adapter.py (operator-implemented)
...
- Tool homepage: search "Mailtrain"
