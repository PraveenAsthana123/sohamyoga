...
> Open-source Python framework for building voice/multimodal AI bots · Daily.co · pipeline-style
> Category: Voice Agent Platform · License: BSD-2 · Port: none
...
| Use Pipecat when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> pipecat["Pipecat"]
    pipecat --> Out["Audio / Text out"]
    pipecat --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool pipecat
```
...
OPENAI_API_KEY · DAILY_TOKEN

## 4. Integration with §91 stack

| §91 layer | Default | With Pipecat |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Pipecat plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Pipecat** |

### Wiring into LangGraph node

```python
# ai-agents/pipecat/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set OPENAI_API_KEY · DAILY_TOKEN · refresh from dashboard |
