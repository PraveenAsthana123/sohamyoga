...
> Best-in-class TTS · voice cloning · multilingual · streaming
...
| Use ElevenLabs when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> elevenlabs["ElevenLabs"]
    elevenlabs --> Out["Audio / Text out"]
    elevenlabs --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool elevenlabs
```
...
| §91 layer | Default | With ElevenLabs |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | ElevenLabs plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **ElevenLabs** |

### Wiring into LangGraph node

```python
# ai-agents/elevenlabs/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set ELEVENLABS_API_KEY · refresh from dashboard |
...
- Tool homepage: (search) ElevenLabs
