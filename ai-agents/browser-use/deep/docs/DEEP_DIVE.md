...
> LLM-driven browser automation via Playwright. Drop-in alternative to raw CDP.
...
| Use Browser-Use when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> browser-use["Browser-Use"]
    browser-use --> Audit["§38.3 audit row"]
    browser-use --> Output["downstream result"]
```
...
```bash
pip install browser-use playwright && python -m playwright install chromium
```
...
### How Browser-Use composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With Browser-Use |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does Browser-Use replace?) |
...
```python
# ai-agents/browser-use/deep/backend/<adapter>.py (operator-implemented)
