...
> Stateful agent DAG. Cycles · checkpointing · HITL gates. Per §64.43 #5 + #10.
...
| Use LangGraph when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> langgraph["LangGraph"]
    langgraph --> Audit["§38.3 audit row"]
    langgraph --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool langgraph
```

## Integration with §91 stack

### How LangGraph composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With LangGraph |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does LangGraph replace?) |
...
```python
# ai-agents/langgraph/deep/backend/<adapter>.py (operator-implemented)
