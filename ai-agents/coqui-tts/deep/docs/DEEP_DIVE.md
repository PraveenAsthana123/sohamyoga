...
> OSS multilingual TTS · XTTS-v2 voice cloning · ~20 languages
> Category: TTS · License: MPL-2.0 · Port: none

## 1. Overview + when-to-use

OSS multilingual TTS · XTTS-v2 voice cloning · ~20 languages

### When to use Coqui TTS

| Use Coqui TTS when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> coqui-tts["Coqui TTS"]
    coqui-tts --> Out["Audio / Text out"]
    coqui-tts --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool coqui-tts
```
...
| §91 layer | Default | With Coqui TTS |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Coqui TTS plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Coqui TTS** |

### Wiring into LangGraph node

```python
# ai-agents/coqui-tts/deep/backend/adapter.py (operator-implemented)
...
- Tool homepage: (search) Coqui TTS
