# AgentOps SDK · Deep Dive

> Agent observability · per-agent trace · token cost · latency · errors.
> License: MIT · Port: (cloud · or self-host)
...
### When to use AgentOps SDK

| Use AgentOps SDK when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> agentops["AgentOps SDK"]
    agentops --> Audit["§38.3 audit row"]
    agentops --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool agentops
```

## Integration with §91 stack

### How AgentOps SDK composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With AgentOps SDK |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does AgentOps SDK replace?) |
...
```python
# ai-agents/agentops/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on (cloud · or self-host) | `docker compose down` or change port via env |
