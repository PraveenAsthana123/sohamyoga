...
> Managed real-time voice agent platform · low-latency · phone/web SDK
> Category: Voice Agent Platform · License: SaaS · Port: none
...
| Use Retell AI when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> retell-ai["Retell AI"]
    retell-ai --> Out["Audio / Text out"]
    retell-ai --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool retell-ai
```
...
| §91 layer | Default | With Retell AI |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Retell AI plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Retell AI** |

### Wiring into LangGraph node

```python
# ai-agents/retell-ai/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set RETELL_API_KEY · refresh from dashboard |
...
- Tool homepage: (search) Retell AI
