...
> OSS mail server · self-hosted SendGrid-alternative · DKIM · bounce handling
> Category: Email Server · License: MIT · Port: 5000
...
| Use Postal when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> postal["Postal"]
    postal --> Out["Output (file / API / DB / queue)"]
    postal --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool postal
```
...
| §91 layer | Default | With Postal |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Postal plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Postal** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/postal/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check admin email |
