...
> Node-based Stable Diffusion · most flexible · custom workflows · best for complex pipelines
> Category: Image Generation · License: GPL-3 · Port: 8188
...
| Use ComfyUI when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> comfyui["ComfyUI"]
    comfyui --> Out["Output (file / API / DB / queue)"]
    comfyui --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool comfyui
```
...
| §91 layer | Default | With ComfyUI |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | ComfyUI plugs in as a tool/connector |
| Side-effect channel | (per use case) | **ComfyUI** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/comfyui/deep/backend/adapter.py (operator-implemented)
