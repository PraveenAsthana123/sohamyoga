...
> OSS social media management · self-hosted Buffer-alternative · multi-platform scheduling
> Category: Social Media Management · License: MIT · Port: 8080
...
| Use Mixpost when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Data / Event source"] --> mixpost["Mixpost"]
    mixpost --> Out["Reports / Dashboards / Workflows"]
    mixpost --> Audit["§38.3 audit row"]
    mixpost --> Vector["§87.4 vector ingest (when applicable)"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool mixpost
```
...
DB + social-platform OAuth

## 4. Integration with §91 stack

| §91 layer | Default | With Mixpost |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Mixpost plugs in as a tool/data-source |
| Side-effect channel | (per use case) | **Mixpost** |

### Wiring into LangGraph node

```python
# ai-agents/mixpost/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| Auth fail | Missing env var or expired token | Check DB + social-platform OAuth |
