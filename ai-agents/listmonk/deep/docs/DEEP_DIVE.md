...
> High-performance newsletter + mailing list manager · self-hosted · Go
> Category: Email/Newsletter · License: AGPL-3 · Port: 9000
...
| Use Listmonk when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> listmonk["Listmonk"]
    listmonk --> Out["Output (file / API / DB / queue)"]
    listmonk --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool listmonk
```
...
| §91 layer | Default | With Listmonk |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Listmonk plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Listmonk** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/listmonk/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check SMTP creds |
