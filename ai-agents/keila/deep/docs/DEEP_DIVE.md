...
> OSS Elixir-based newsletter platform · GDPR-friendly · self-hosted Mailchimp-alternative
> Category: Email/Newsletter · License: MIT · Port: 4000
...
| Use Keila when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> keila["Keila"]
    keila --> Out["Output (file / API / DB / queue)"]
    keila --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool keila
```
...
| §91 layer | Default | With Keila |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Keila plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Keila** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/keila/deep/backend/adapter.py (operator-implemented)
