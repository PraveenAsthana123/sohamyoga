# Cartesia (Sonic) · Deep Dive

> Low-latency TTS · streaming · 90ms p50 · Sonic-Turbo model
> Category: TTS · License: SaaS · Port: none

## 1. Overview + when-to-use

Low-latency TTS · streaming · 90ms p50 · Sonic-Turbo model

### When to use Cartesia (Sonic)

| Use Cartesia (Sonic) when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> cartesia["Cartesia (Sonic)"]
    cartesia --> Out["Audio / Text out"]
    cartesia --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool cartesia
```
...
| §91 layer | Default | With Cartesia (Sonic) |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Cartesia (Sonic) plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Cartesia (Sonic)** |

### Wiring into LangGraph node

```python
# ai-agents/cartesia/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set CARTESIA_API_KEY · refresh from dashboard |
...
- Tool homepage: (search) Cartesia (Sonic)
