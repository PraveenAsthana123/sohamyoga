...
> Autonomous coding agent · CodeAct framework · sandboxed execution.
> License: MIT · Port: 3000
...
| Use OpenHands when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> openhands["OpenHands"]
    openhands --> Audit["§38.3 audit row"]
    openhands --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool openhands
```

## Integration with §91 stack

### How OpenHands composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With OpenHands |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does OpenHands replace?) |
...
```python
# ai-agents/openhands/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on 3000 | `docker compose down` or change port via env |
