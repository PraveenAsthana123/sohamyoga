...
> Simplified Stable Diffusion UI · SDXL · auto-prompt enhancement · one-click
> Category: Image Generation · License: GPL-3 · Port: 7865
...
| Use Fooocus when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> fooocus["Fooocus"]
    fooocus --> Out["Output (file / API / DB / queue)"]
    fooocus --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool fooocus
```
...
| §91 layer | Default | With Fooocus |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Fooocus plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Fooocus** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/fooocus/deep/backend/adapter.py (operator-implemented)
