# Formbricks · Deep Dive

> OSS in-product survey + NPS + experience management · React widget
> Category: Survey/Forms · License: AGPL-3 (community) · Port: 3000

## 1. Overview + when-to-use

OSS in-product survey + NPS + experience management · React widget

### When to use Formbricks

| Use Formbricks when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> formbricks["Formbricks"]
    formbricks --> Out["Output (file / API / DB / queue)"]
    formbricks --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool formbricks
```
...
| §91 layer | Default | With Formbricks |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Formbricks plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Formbricks** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/formbricks/deep/backend/adapter.py (operator-implemented)
...
- Tool homepage: search "Formbricks"
