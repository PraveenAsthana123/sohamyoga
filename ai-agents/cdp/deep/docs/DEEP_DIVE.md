# Chrome DevTools Protocol · Deep Dive

> Programmatic browser control. DOM · screenshot · click · execute JS · network log.
> License: — · Port: 9222
...
| Use Chrome DevTools Protocol when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Caller["§91 LangGraph DAG"] --> cdp["Chrome DevTools Protocol"]
    cdp --> Audit["§38.3 audit row"]
    cdp --> Output["downstream result"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool cdp
```

## Integration with §91 stack

### How Chrome DevTools Protocol composes with WebLLM + CDP + RAG + LangGraph

| §91 layer | Default | With Chrome DevTools Protocol |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP raw | (operator fills · what does Chrome DevTools Protocol replace?) |
...
```python
# ai-agents/cdp/deep/backend/<adapter>.py (operator-implemented)
...
|---|---|---|
| Port already in use | Another instance on 9222 | `docker compose down` or change port via env |
