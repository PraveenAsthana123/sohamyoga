# AssemblyAI · Deep Dive

> STT with features · sentiment · summarization · entity detection
> Category: STT · License: SaaS · Port: none
...
| Use AssemblyAI when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> assemblyai["AssemblyAI"]
    assemblyai --> Out["Audio / Text out"]
    assemblyai --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool assemblyai
```
...
| §91 layer | Default | With AssemblyAI |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | AssemblyAI plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **AssemblyAI** |

### Wiring into LangGraph node

```python
# ai-agents/assemblyai/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set ASSEMBLYAI_API_KEY · refresh from dashboard |
...
- Tool homepage: (search) AssemblyAI
