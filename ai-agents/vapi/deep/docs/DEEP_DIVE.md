...
> Managed voice AI platform · phone calls + web · agent orchestration
...
| Use Vapi when... | Use alternative when... |
|---|---|
...
graph LR
    ...
    Stream --> vapi["Vapi"]
    vapi --> Out["Audio / Text out"]
    vapi --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool vapi
```
...
| §91 layer | Default | With Vapi |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Vapi plugs into LangGraph as a voice tool |
| Audio I/O | (none in §91 default) | **Vapi** |

### Wiring into LangGraph node

```python
# ai-agents/vapi/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| API 401 / unauthorized | Missing or expired API key | Set VAPI_API_KEY · refresh from dashboard |
