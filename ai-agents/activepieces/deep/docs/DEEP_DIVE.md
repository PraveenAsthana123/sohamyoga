...
> OSS workflow automation · Zapier/Make-alternative · 200+ connectors · self-hosted
> Category: Workflow Automation · License: MIT (community) + commercial enterprise · Port: 8080
...
| Use Activepieces when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Data / Event source"] --> activepieces["Activepieces"]
    activepieces --> Out["Reports / Dashboards / Workflows"]
    activepieces --> Audit["§38.3 audit row"]
    activepieces --> Vector["§87.4 vector ingest (when applicable)"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool activepieces
```
...
| §91 layer | Default | With Activepieces |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Activepieces plugs in as a tool/data-source |
| Side-effect channel | (per use case) | **Activepieces** |

### Wiring into LangGraph node

```python
# ai-agents/activepieces/deep/backend/adapter.py (operator-implemented)
...
|---|---|---|
| Auth fail | Missing env var or expired token | Check AP_API_KEY |
...
- Tool homepage: search "Activepieces"
