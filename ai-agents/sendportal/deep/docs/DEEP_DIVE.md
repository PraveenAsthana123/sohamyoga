# SendPortal · Deep Dive

> OSS Laravel email-newsletter app · SES integration · campaign automation
> Category: Email/Newsletter · License: MIT · Port: 8000

## 1. Overview + when-to-use

OSS Laravel email-newsletter app · SES integration · campaign automation

### When to use SendPortal

| Use SendPortal when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> sendportal["SendPortal"]
    sendportal --> Out["Output (file / API / DB / queue)"]
    sendportal --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool sendportal
```
...
| §91 layer | Default | With SendPortal |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | SendPortal plugs in as a tool/connector |
| Side-effect channel | (per use case) | **SendPortal** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/sendportal/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check SES creds |
...
- Tool homepage: search "SendPortal"
