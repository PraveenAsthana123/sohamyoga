...
> JS library · form builder + survey rendering · React/Vue/Angular · self-hosted
> Category: Survey/Forms · License: MIT (Library) + commercial (Builder) · Port: none
...
| Use SurveyJS when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> surveyjs["SurveyJS"]
    surveyjs --> Out["Output (file / API / DB / queue)"]
    surveyjs --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool surveyjs
```
...
| §91 layer | Default | With SurveyJS |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | SurveyJS plugs in as a tool/connector |
| Side-effect channel | (per use case) | **SurveyJS** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/surveyjs/deep/backend/adapter.py (operator-implemented)
