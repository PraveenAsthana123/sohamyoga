# AUTOMATIC1111 SD WebUI · Deep Dive

> Stable Diffusion WebUI · most extensions · LoRA · ControlNet · plugin ecosystem
> Category: Image Generation · License: AGPL-3 · Port: 7860

## 1. Overview + when-to-use

Stable Diffusion WebUI · most extensions · LoRA · ControlNet · plugin ecosystem

### When to use AUTOMATIC1111 SD WebUI

| Use AUTOMATIC1111 SD WebUI when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> stable-diffusion-webui["AUTOMATIC1111 SD WebUI"]
    stable-diffusion-webui --> Out["Output (file / API / DB / queue)"]
    stable-diffusion-webui --> Audit["§38.3 audit row"]
```
...
| §91 layer | Default | With AUTOMATIC1111 SD WebUI |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | AUTOMATIC1111 SD WebUI plugs in as a tool/connector |
| Side-effect channel | (per use case) | **AUTOMATIC1111 SD WebUI** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/stable-diffusion-webui/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check none |
...
- Tool homepage: search "AUTOMATIC1111 SD WebUI"
