...
> Image manipulation (raster) · scripting · GEGL pipeline · plugin ecosystem
> Category: Image Editing · License: GPL-3 · Port: none
...
| Use GIMP when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> gimp["GIMP"]
    gimp --> Out["Output (file / API / DB / queue)"]
    gimp --> Audit["§38.3 audit row"]
```
...
```bash
./scripts/setup_ai_agent_stack.sh --tool gimp
```
...
| §91 layer | Default | With GIMP |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | GIMP plugs in as a tool/connector |
| Side-effect channel | (per use case) | **GIMP** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/gimp/deep/backend/adapter.py (operator-implemented)
