...
> Category: Marketing Automation · License: GPL-3 · Port: 8080
...
| Use Mautic when... | Use alternative when... |
|---|---|
...
```mermaid
graph LR
    Source["Input / Trigger"] --> mautic["Mautic"]
    mautic --> Out["Output (file / API / DB / queue)"]
    mautic --> Audit["§38.3 audit row"]
```
...
| §91 layer | Default | With Mautic |
|---|---|---|
| LLM in browser | WebLLM | unchanged |
| Browser control | CDP | unchanged |
| Retrieval | Chroma RAG | unchanged |
| Orchestration | LangGraph | Mautic plugs in as a tool/connector |
| Side-effect channel | (per use case) | **Mautic** (image / form / email) |

### Wiring into LangGraph node

```python
# ai-agents/mautic/deep/backend/adapter.py (operator-implemented)
...
| Auth fail | Missing env var or expired token | Check DB + SMTP |
