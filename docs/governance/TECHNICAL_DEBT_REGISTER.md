...
| TD-07 | Observability | No monitoring/alerting for process-death or infra-staleness | High | All 3 real incidents this audit found were invisible to existing systems | N/A | Low-Medium (external health poller) | P1 | OBS-01 | **Closed** — real `scripts/health-monitor.sh` checking 12 real endpoints across all portals/gateways/models, scheduled every 10min via systemd timer, verified live (12/12 pass after fixing one false-positive test expectation) |
...
| TD-09 | Testing | voice-agent-platform's proven tenant-isolation guard has zero regression test | High | Could silently regress a fix for an incident that already happened once | High | Low (one test, logic already understood) | **P0** | COVERAGE_GAP_MATRIX SEC-06 | **Closed** — Jest added (this portal had zero test infra), 3 real tests written, verified non-vacuous by temporarily neutering the real guard and confirming the suite failed |
| TD-10 | AI/LLMOps | Zero cost/token/latency tracking on any AI feature | Medium | Unknown AI spend, no capacity planning possible | N/A | Low (extend shared OllamaClient) | P2 | OBS-01 (AI) | **Closed** — latency/token counts now surfaced from Ollama's own response data in the shared client, covers both consuming portals; verified live (real 18921ms/29/2 tokens on an actual call) |
...
| TD-14 | Security | ai-orchestrator-platform's DocxViewer has zero HTML sanitization | Medium | Low likelihood (single-operator, trusted-root threat model) | Medium | Low (add DOMPurify, template exists in sohamyoga-frontend) | P2 | SEC-08 | **Closed** — DOMPurify added, verified via `tsc --noEmit` |
| TD-15 | Reliability | voice-agent-platform's real call placement has no rate limit | High if ever exposed further | Cost-abuse vector | Medium | Low | P2 | SEC-06 | **Closed** — same rate-limit pattern as market-research-portal applied (10/min per IP), verified via `tsc --noEmit` |
...
| TD-23 | Security/Reliability | **password-manager's `argon2` native module segfaults on load** — discovered running a real e2e test attempt; the app cannot complete signup/login at all on this host | **Critical** | The app is completely non-functional, not just untested | Critical — the entire auth/crypto system cannot run | Unknown (root cause not isolated — reinstall + rebuild both failed the same way) | **P0** | Reality Matrix update, SEC-17 | Open — newly discovered this session |
| TD-24 | Security | ai-orchestrator-platform's cookie `secure` flag was hardcoded `False` even over the public HTTPS tunnel | Medium | N/A | Medium | Low | P2 | Reality Matrix Phase 1 | **Closed** — now derived from `X-Forwarded-Proto`, verified live (backend restarted clean, `/auth/status` 200) |
| TD-25 | Security | voice-agent-platform had no `/api/health` route at all (the one portal missing one) | Low-Medium | Made this exact container's earlier outage harder to detect externally | N/A | Low | P2 | PER_PORTAL_SCORECARD.md | **Closed** — real DB-connectivity health route added, verified live |
| TD-26 | Security | market-research-portal's `/api/auth/login` was the one auth endpoint without rate limiting (leads/capture and intake already had it) | Medium | Credential-stuffing exposure | Medium | Low | P2 | THREAT_MODEL.md | **Closed** — same limiter pattern applied (10/min per IP), rebuilt, verified live (11th request in one minute returns 429) |
| TD-27 | Architecture | **Duplicate AI gateway infrastructure**: `litellm-gateway.service` (built this session, verified end-to-end against local Ollama) and a pre-existing `omniroute.service` (npm package, real, actively published, started ~2h earlier the same day by a different/parallel session — not this one) both run on this machine, both claiming to be a unified OpenAI-compatible multi-provider AI router. Neither was aware of the other at setup time. | Medium | Confusing which gateway is "the real one" for any future integration work; wasted resources (2.8GB + a Python venv for overlapping capability) | Low (no security issue, just duplication) | Low to decide, unknown to migrate depending on choice | P2 | This session's live discovery, `LITELLM_GATEWAY_SETUP.md` | **Open — explicit decision: leave both running for now**, revisit once it's clear why OmniRoute exists and what's configured behind it |
