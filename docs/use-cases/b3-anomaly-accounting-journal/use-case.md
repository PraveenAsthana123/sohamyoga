# Use Case · Anomaly Accounting Journal

> **Block** · B3 · **Dept** · 14 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 14 Finance · IsoForest+VAE ensemble
...
|---|---|---|---|
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-ANOMALY_ACCOUNTING_JOURNAL-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
