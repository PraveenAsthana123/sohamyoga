# Use Case · Lstm Payment Failure Sequence

> **Block** · C3 · **Dept** · 6 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 6 Billing · Bi-LSTM churn risk
...
|---|---|---|---|
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-LSTM_PAYMENT_FAILURE_SEQUENCE-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
