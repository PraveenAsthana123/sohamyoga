# Use Case · Bayesian Optimization Hp Search

> **Block** · E6 · **Dept** · 18 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

All depts · Optuna BO + Ax
...
|---|---|---|---|
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-BAYESIAN_OPTIMIZATION_HP_SEARCH-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
