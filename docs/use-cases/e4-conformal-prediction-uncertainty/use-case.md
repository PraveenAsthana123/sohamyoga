# Use Case · Conformal Prediction Uncertainty

> **Block** · E4 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 UW · distribution-free CI
...
|---|---|---|---|
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CONFORMAL_PREDICTION_UNCERTAINTY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
