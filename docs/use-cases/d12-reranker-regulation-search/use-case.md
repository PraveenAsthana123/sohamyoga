# Use Case · Reranker Regulation Search

> **Block** · D12 · **Dept** · 12 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 12 Compliance · cross-encoder rerank
...
|---|---|---|---|
| `INSUR-RERANKER_REGULATION_SEARCH-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RERANKER_REGULATION_SEARCH-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RERANKER_REGULATION_SEARCH-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RERANKER_REGULATION_SEARCH-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-RERANKER_REGULATION_SEARCH-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-RERANKER_REGULATION_SEARCH-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
