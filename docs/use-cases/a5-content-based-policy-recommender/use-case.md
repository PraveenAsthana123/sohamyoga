# Use Case · Content Based Policy Recommender

> **Block** · A5 · **Dept** · 3 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 3 Sales · attribute-similarity rec
...
|---|---|---|---|
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CONTENT_BASED_POLICY_RECOMMENDER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
