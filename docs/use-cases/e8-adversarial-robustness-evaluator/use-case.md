# Use Case · Adversarial Robustness Evaluator

> **Block** · E8 · **Dept** · 20 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Cyber + CV · FGSM/PGD audit
...
|---|---|---|---|
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-ADVERSARIAL_ROBUSTNESS_EVALUATOR-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
