# Use Case · Federated Cross Tenant Fraud

> **Block** · D4 · **Dept** · 8 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Cross-tenant · FedAvg w/ DP
...
|---|---|---|---|
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-FEDERATED_CROSS_TENANT_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
