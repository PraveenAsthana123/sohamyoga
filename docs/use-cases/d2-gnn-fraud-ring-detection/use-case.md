# Use Case · Gnn Fraud Ring Detection

> **Block** · D2 · **Dept** · 8 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 8 SIU · GraphSAGE on entity graph
...
|---|---|---|---|
| `INSUR-GNN_FRAUD_RING_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-GNN_FRAUD_RING_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-GNN_FRAUD_RING_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-GNN_FRAUD_RING_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-GNN_FRAUD_RING_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-GNN_FRAUD_RING_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
