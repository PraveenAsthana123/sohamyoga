# Use Case · Knowledge Graph Regulation Reasoner

> **Block** · D8 · **Dept** · 13 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 13 Legal · Neo4j + GNN + rules
...
|---|---|---|---|
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-KNOWLEDGE_GRAPH_REGULATION_REASONER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
