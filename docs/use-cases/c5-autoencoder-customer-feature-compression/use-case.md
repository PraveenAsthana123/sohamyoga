# Use Case · Autoencoder Customer Feature Compression

> **Block** · C5 · **Dept** · 18 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 18 Analytics · 800→32 AE
...
|---|---|---|---|
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-AUTOENCODER_CUSTOMER_FEATURE_COMPRESSION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
