# Use Case · Hybrid Ml Cv Nlp Rag Multimodal

> **Block** · F4 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · full multimodal assistant
...
|---|---|---|---|
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-HYBRID_ML_CV_NLP_RAG_MULTIMODAL-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
