# Use Case · Claims Total Loss Determination

> **Block** · B4 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · hybrid XGB+CNN
...
|---|---|---|---|
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CLAIMS_TOTAL_LOSS_DETERMINATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
