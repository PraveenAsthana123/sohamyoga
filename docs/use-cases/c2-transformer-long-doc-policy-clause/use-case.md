# Use Case · Transformer Long Doc Policy Clause

> **Block** · C2 · **Dept** · 13 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 13 Legal · Longformer clause class
...
|---|---|---|---|
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-TRANSFORMER_LONG_DOC_POLICY_CLAUSE-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
