# Use Case · Collaborative Agent Skill Routing

> **Block** · B2 · **Dept** · 9 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 9 CS · ALS skill match
...
|---|---|---|---|
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-COLLABORATIVE_AGENT_SKILL_ROUTING-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
