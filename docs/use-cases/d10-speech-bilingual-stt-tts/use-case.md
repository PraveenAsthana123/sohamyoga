# Use Case · Speech Bilingual Stt Tts

> **Block** · D10 · **Dept** · 9 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 9 CS · Whisper + Coqui-TTS
...
|---|---|---|---|
| `INSUR-SPEECH_BILINGUAL_STT_TTS-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SPEECH_BILINGUAL_STT_TTS-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SPEECH_BILINGUAL_STT_TTS-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SPEECH_BILINGUAL_STT_TTS-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-SPEECH_BILINGUAL_STT_TTS-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-SPEECH_BILINGUAL_STT_TTS-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
