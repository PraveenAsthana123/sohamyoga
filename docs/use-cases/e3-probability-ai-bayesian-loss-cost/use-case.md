# Use Case · Probability Ai Bayesian Loss Cost

> **Block** · E3 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 UW · PyMC hierarchical Bayes
...
|---|---|---|---|
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-PROBABILITY_AI_BAYESIAN_LOSS_COST-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
