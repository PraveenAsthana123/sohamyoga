# Use Case · Explainability As Service

> **Block** · D18 · **Dept** · 18 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

All depts · SHAP/LIME/IG/Captum service
...
|---|---|---|---|
| `INSUR-EXPLAINABILITY_AS_SERVICE-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-EXPLAINABILITY_AS_SERVICE-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-EXPLAINABILITY_AS_SERVICE-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-EXPLAINABILITY_AS_SERVICE-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-EXPLAINABILITY_AS_SERVICE-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-EXPLAINABILITY_AS_SERVICE-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
