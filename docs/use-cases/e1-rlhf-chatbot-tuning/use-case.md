# Use Case · Rlhf Chatbot Tuning

> **Block** · E1 · **Dept** · 9 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 9 CS · RLHF on agent preferences
...
|---|---|---|---|
| `INSUR-RLHF_CHATBOT_TUNING-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RLHF_CHATBOT_TUNING-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RLHF_CHATBOT_TUNING-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RLHF_CHATBOT_TUNING-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-RLHF_CHATBOT_TUNING-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-RLHF_CHATBOT_TUNING-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
