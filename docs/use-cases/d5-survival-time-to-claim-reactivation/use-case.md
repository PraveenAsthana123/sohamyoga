# Use Case · Survival Time To Claim Reactivation

> **Block** · D5 · **Dept** · 11 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 11 Reinsurance · DeepSurv
...
|---|---|---|---|
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-SURVIVAL_TIME_TO_CLAIM_REACTIVATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
