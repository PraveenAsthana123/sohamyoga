# Use Case · Neural Ode Irregular Time Series

> **Block** · E7 · **Dept** · 16 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Health/Life dept · latent ODE
...
|---|---|---|---|
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-NEURAL_ODE_IRREGULAR_TIME_SERIES-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
