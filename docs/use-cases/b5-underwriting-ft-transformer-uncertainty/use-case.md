# Use Case · Underwriting Ft Transformer Uncertainty

> **Block** · B5 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 UW · FT-Transformer + MC-Dropout
...
|---|---|---|---|
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-UNDERWRITING_FT_TRANSFORMER_UNCERTAINTY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
