# Use Case · Self Supervised Customer Embedding

> **Block** · D6 · **Dept** · 18 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

All depts · SimCLR contrastive pretrain
...
|---|---|---|---|
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-SELF_SUPERVISED_CUSTOMER_EMBEDDING-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
