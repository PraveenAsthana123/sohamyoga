# Use Case · Counterfactual Denial Explanation

> **Block** · D15 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 UW · DiCE counterfactual
...
|---|---|---|---|
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-COUNTERFACTUAL_DENIAL_EXPLANATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
