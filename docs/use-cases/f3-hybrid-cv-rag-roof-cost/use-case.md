# Use Case · Hybrid Cv Rag Roof Cost

> **Block** · F3 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · U-Net + regional cost RAG
...
|---|---|---|---|
| `INSUR-HYBRID_CV_RAG_ROOF_COST-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HYBRID_CV_RAG_ROOF_COST-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HYBRID_CV_RAG_ROOF_COST-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HYBRID_CV_RAG_ROOF_COST-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-HYBRID_CV_RAG_ROOF_COST-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-HYBRID_CV_RAG_ROOF_COST-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
