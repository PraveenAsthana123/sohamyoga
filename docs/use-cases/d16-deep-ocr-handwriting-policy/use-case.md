# Use Case · Deep Ocr Handwriting Policy

> **Block** · D16 · **Dept** · 5 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 5 Policy Admin · TrOCR
...
|---|---|---|---|
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-DEEP_OCR_HANDWRITING_POLICY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
