# Use Case · Hybrid Agentic Rag Mcp Workflow

> **Block** · F5 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · §64.40 10-layer + Temporal
...
|---|---|---|---|
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-HYBRID_AGENTIC_RAG_MCP_WORKFLOW-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
