# Use Case · Cv Segmentation Roof Damage

> **Block** · B1 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · U-Net per-pixel mask
...
|---|---|---|---|
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CV_SEGMENTATION_ROOF_DAMAGE-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
