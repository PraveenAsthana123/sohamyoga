# Use Case · Topic Drift Customer Complaints

> **Block** · D13 · **Dept** · 18 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 18 Analytics · BERTopic
...
|---|---|---|---|
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-TOPIC_DRIFT_CUSTOMER_COMPLAINTS-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
