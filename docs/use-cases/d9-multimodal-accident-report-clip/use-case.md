# Use Case · Multimodal Accident Report Clip

> **Block** · D9 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · vision+text CLIP fusion
...
|---|---|---|---|
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-MULTIMODAL_ACCIDENT_REPORT_CLIP-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
