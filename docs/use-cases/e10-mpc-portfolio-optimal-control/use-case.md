# Use Case · Mpc Portfolio Optimal Control

> **Block** · E10 · **Dept** · 14 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 14 Finance · MPC + LSTM forecast
...
|---|---|---|---|
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-MPC_PORTFOLIO_OPTIMAL_CONTROL-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
