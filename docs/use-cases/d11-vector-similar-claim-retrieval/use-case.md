# Use Case · Vector Similar Claim Retrieval

> **Block** · D11 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

All depts · pgvector cosine top-K
...
|---|---|---|---|
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-VECTOR_SIMILAR_CLAIM_RETRIEVAL-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
