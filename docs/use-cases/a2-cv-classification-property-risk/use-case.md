# Use Case · Cv Classification Property Risk

> **Block** · A2 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 Underwriting · ResNet 7-class risk
...
|---|---|---|---|
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CV_CLASSIFICATION_PROPERTY_RISK-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
