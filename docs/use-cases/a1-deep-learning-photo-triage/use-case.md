# Use Case · Deep Learning Photo Triage

> **Block** · A1 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · CNN damage class
...
|---|---|---|---|
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-DEEP_LEARNING_PHOTO_TRIAGE-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
