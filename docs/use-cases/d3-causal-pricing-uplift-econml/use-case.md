# Use Case · Causal Pricing Uplift Econml

> **Block** · D3 · **Dept** · 4 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 4 UW · CATE for premium elasticity
...
|---|---|---|---|
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-CAUSAL_PRICING_UPLIFT_ECONML-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
