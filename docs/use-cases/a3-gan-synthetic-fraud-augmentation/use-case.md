# Use Case · Gan Synthetic Fraud Augmentation

> **Block** · A3 · **Dept** · 8 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 8 SIU · CTGAN augments rare fraud
...
|---|---|---|---|
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-GAN_SYNTHETIC_FRAUD_AUGMENTATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
