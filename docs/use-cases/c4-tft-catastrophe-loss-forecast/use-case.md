# Use Case · Tft Catastrophe Loss Forecast

> **Block** · C4 · **Dept** · 11 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 11 Reinsurance · TFT quantile forecast
...
|---|---|---|---|
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-TFT_CATASTROPHE_LOSS_FORECAST-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
