# Use Case · Hybrid Dl Rag Damage Photo Repair

> **Block** · F2 · **Dept** · 7 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 7 Claims · CNN + OEM manual RAG
...
|---|---|---|---|
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-HYBRID_DL_RAG_DAMAGE_PHOTO_REPAIR-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
