# Use Case · Vae Network Anomaly

> **Block** · A4 · **Dept** · 20 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 20 Cyber · VAE reconstruction anomaly
...
|---|---|---|---|
| `INSUR-VAE_NETWORK_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VAE_NETWORK_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VAE_NETWORK_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VAE_NETWORK_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-VAE_NETWORK_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-VAE_NETWORK_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
