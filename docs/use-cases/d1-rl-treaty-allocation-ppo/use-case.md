# Use Case · Rl Treaty Allocation Ppo

> **Block** · D1 · **Dept** · 11 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 11 Reinsurance · PPO optimal allocation
...
|---|---|---|---|
| `INSUR-RL_TREATY_ALLOCATION_PPO-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RL_TREATY_ALLOCATION_PPO-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RL_TREATY_ALLOCATION_PPO-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RL_TREATY_ALLOCATION_PPO-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-RL_TREATY_ALLOCATION_PPO-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-RL_TREATY_ALLOCATION_PPO-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
