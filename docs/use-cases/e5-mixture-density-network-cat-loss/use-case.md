# Use Case · Mixture Density Network Cat Loss

> **Block** · E5 · **Dept** · 11 · **Status** · stub (operator fills sections)
> Generated from §90 catalog. Edit this file in-place.

## 1. Use case

Dept 11 Reinsurance · MDN multi-modal
...
|---|---|---|---|
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB (per §87.4 + §90.5) | vector_db |
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-HITL-AUDIT` | `0 9 * * *` | sample overrides for retrain | hitl_audit |
| `INSUR-MIXTURE_DENSITY_NETWORK_CAT_LOSS-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metric audit | fairness_audit |
