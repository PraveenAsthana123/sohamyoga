#!/bin/sh
# Seeds all SohamYoga portal secrets into OpenBao at startup.
...
bao secrets enable -path=sohamyoga-portal kv-v2 2>/dev/null || echo "already enabled"

echo "==> Writing portal secrets..."

# Database
bao kv put sohamyoga-portal/database \
  url="${DATABASE_URL}" \
  host="${DB_HOST}" \
  port="${DB_PORT:-5432}" \
  name="${DB_NAME}" \
...
# Ollama
bao kv put sohamyoga-portal/ollama \
