  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">My Ad Campaigns</h1>
          <p className="text-sm text-gray-500 mt-1">Active campaigns running for your business</p>
        </div>
        ...
        {campaigns.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 className="font-semibold text-gray-900 mb-4">Spend vs. Budget</h3>
            <div className="space-y-3">
              {campaigns.map(c => {
                const spendPct = c.budget > 0 ? Math.min((c.spend / (c.budget * 30)) * 100, 100) : 0;
                return (
                  <div key={c.id} className="flex items-center gap-3 text-sm">
                    <span className="w-48 text-gray-700 truncate text-sm">{c.name}</span>
                    ...
                    <span className="text-xs text-gray-500 w-28 text-right">${c.spend.toFixed(0)} of ${c.budget * 30}/mo</span>
                  </div>
                );
              })}
            </div>
                ...
                <div key={c.id} className="border border-gray-100 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">{PLATFORM_LABELS[c.platform] ?? c.platform}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div><p className="text-xs text-gray-400">Budget/day</p><p className="font-semibold text-gray-700">${c.budget}</p></div>
                    <div><p className="text-xs text-gray-400">Impressions</p><p className="font-semibold text-gray-700">{c.impressions?.toLocaleString() ?? '—'}</p></div>
                    <div><p className="text-xs text-gray-400">Clicks</p><p className="font-semibold text-gray-700">{c.clicks?.toLocaleString() ?? '—'}</p></div>
                    <div><p className="text-xs text-gray-400">Leads</p><p className="font-semibold text-green-600">{c.conversions ?? 0}</p></div>
                  </div>
                </div>
              ))}
            </div>
