  ...
  try {
    ...
    const { rows } = await client.query(
      `UPDATE pipeline_health
       SET last_run = NOW(), last_failure = NOW(), run_count = run_count + 1, failure_count = failure_count + 1, status = 'failing'
       WHERE pipeline_name = $1
       RETURNING *`,
      [name]
    );
    if (rows.length === 0) return Response.json({ error: 'Pipeline not found' }, { status: 404 });
    return Response.json({ pipeline: rows[0], message: 'Failure recorded' });
  } finally {
    client.release();
  }
}
