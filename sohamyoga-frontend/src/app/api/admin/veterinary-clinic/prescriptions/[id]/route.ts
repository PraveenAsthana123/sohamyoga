...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }): Promise<Response> {
  ...
  try {
    ...
    const fields = ['dispensed','refills_remaining','dosage','frequency','duration_days','notes','prescribed_by'];
    const sets: string[] = [];
    const vals: unknown[] = [];
    ...
    try {
      const { rows } = await client.query(`UPDATE vet_prescription SET ${sets.join(',')} WHERE id=$${vals.length} RETURNING *`, vals);
