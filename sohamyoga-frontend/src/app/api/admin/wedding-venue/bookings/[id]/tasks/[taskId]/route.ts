...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; taskId: string } }): Promise<Response> {
  ...
  try {
    const allowed = ['task_name','task_category','due_date','assigned_to','status','priority','notes'];
    const sets: string[] = [];
    ...
    const r = await client.query(
      `UPDATE wv_planning_task SET ${sets.join(',')} WHERE id=$${values.length-1} AND booking_id=$${values.length} RETURNING *`,
      values
    );
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string; taskId: string } }): Promise<Response> {
  ...
  const client = await pool.connect();
  try {
    await client.query(`DELETE FROM wv_planning_task WHERE id=$1 AND booking_id=$2`, [params.taskId, params.id]);
