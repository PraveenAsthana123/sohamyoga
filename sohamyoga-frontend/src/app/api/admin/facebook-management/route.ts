...
export async function GET() {
  ...
  try {
    const [accountRes, contentRes, draftsRes, analyticsRes, scheduleRes] = await Promise.all([
      client.query(
        `SELECT id, platform, account_name, profile_url, avatar_url, status,
                connected_at, last_health_check_at, error_message
         FROM social_account WHERE platform = 'facebook' ORDER BY connected_at DESC LIMIT 1`
      ).catch(() => ({ rows: [] })),
      ...
      client.query(
        `SELECT id, platform, content_type, caption AS title, status, approval_status,
         ...
         FROM unified_content_item WHERE platform = 'facebook'
      ...
      client.query(
        `SELECT id, content_type, master_text, status, default_schedule_at,
         ...
         FROM social_content_draft WHERE platform ILIKE '%facebook%'
         ORDER BY created_at DESC LIMIT 100`
      ).catch(() => ({ rows: [] })),

      client.query(
        `SELECT id, platform, metric_key, metric_value, recorded_at
         FROM social_platform_analytics WHERE platform = 'facebook'
