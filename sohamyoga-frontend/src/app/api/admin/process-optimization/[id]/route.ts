...
export async function GET(req: NextRequest, { params }: { params: { id: string } }): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ initiative: r.rows[0] });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }): Promise<Response> {
  ...
  const allowed = ['name','methodology','process_name','current_metric','target_metric','unit','status','owner','savings_usd'] as const;
  const fields: string[] = [];
  const vals: unknown[] = [];
  let i = 1;
  ...
  const client = await pool.connect();
  try {
    const r = await client.query(`UPDATE optimization_initiatives SET ${fields.join(',')} WHERE id=$${i} RETURNING *`, vals);
    if (!r.rowCount) return Response.json({ error: 'Not found' }, { status: 404 });
    return Response.json({ initiative: r.rows[0] });
  } finally { client.release(); }
}
