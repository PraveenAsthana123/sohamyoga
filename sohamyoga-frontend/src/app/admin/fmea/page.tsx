...
interface MainRow {
  ...
  failure_mode: string | number | boolean | string[] | null;
  effect: string | number | boolean | string[] | null;
  cause: string | number | boolean | string[] | null;
  severity: string | number | boolean | string[] | null;
  occurrence: string | number | boolean | string[] | null;
  detection: string | number | boolean | string[] | null;
  current_controls: string | number | boolean | string[] | null;
  recommended_action: string | number | boolean | string[] | null;
  action_owner: string | number | boolean | string[] | null;
...
export default function FmeaPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "process_name": "", "failure_mode": "", "effect": "", "cause": "", "severity": "", "occurrence": "", "detection": "", "current_controls": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Calculate RPN scores and prioritize failure modes for risk mitigation');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/fmea');
      if (!res.ok) { setError('Failed to load data'); return; }
      const d = await res.json() as { items: MainRow[] };
      setItems(d.items ?? []);
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/fmea?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Failure Mode & Effects Analysis', secondary: 'Analytics', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">⚠️ Failure Mode & Effects Analysis</h1>
          <p className="text-gray-500 text-sm mt-1">Manage failure mode & effects analysis data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Failure Mode & Effects Analysis items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Occurrence</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.failure_mode ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.effect ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.cause ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.severity ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.occurrence ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-orange-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-orange-700">{items.filter(i => String(i.status) !== 'active').length}</div>
              <div className="text-sm text-orange-600">Inactive / Other</div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your failure mode & effects analysis data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Failure Mode & Effects Analysis</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.effect} onChange={e => setForm(f => ({...f, effect: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Cause</label>
                <input type="text" value={form.cause} onChange={e => setForm(f => ({...f, cause: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Severity</label>
                <input type="text" value={form.severity} onChange={e => setForm(f => ({...f, severity: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Occurrence</label>
                <input type="text" value={form.occurrence} onChange={e => setForm(f => ({...f, occurrence: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Detection</label>
                <input type="text" value={form.detection} onChange={e => setForm(f => ({...f, detection: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Current Controls</label>
                <input type="text" value={form.current_controls} onChange={e => setForm(f => ({...f, current_controls: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
