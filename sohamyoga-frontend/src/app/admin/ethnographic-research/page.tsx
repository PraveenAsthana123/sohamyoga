...
interface MainRow {
  id: string;
    study_name: string | number | boolean | string[] | null;
  objective: string | number | boolean | string[] | null;
  method: string | number | boolean | string[] | null;
  client_name: string | number | boolean | string[] | null;
  lead_researcher: string | number | boolean | string[] | null;
  participant_count: string | number | boolean | string[] | null;
  field_days: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  study_id: string;
    participant_code: string | number | boolean | string[] | null;
  observation_type: string | number | boolean | string[] | null;
  observation_text: string | number | boolean | string[] | null;
  context: string | number | boolean | string[] | null;
  recorded_at: string | number | boolean | string[] | null;
  created_at: string;
}

export default function EthnographicResearchPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "study_name": "", "objective": "", "method": "", "client_name": "", "lead_researcher": "", "participant_count": "", "field_days": "", "location": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze ethnographic research observations and synthesize behavioral insights');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/ethnographic-research');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/ethnographic-research?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Ethnographic Research', secondary: 'Ethnographic Observation', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🔭 Ethnographic Research</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ethnographic research data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Ethnographic Research items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Study Name</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Lead Researcher</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.study_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.objective ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.method ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.client_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.lead_researcher ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Ethnographic Observation</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Ethnographic Research Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No ethnographic observation records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Participant Code</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Observation Type</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Observation Text</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.participant_code ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.observation_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.observation_text ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.context ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your ethnographic research data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Ethnographic Research</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Study Name</label>
                <input type="text" value={form.study_name} onChange={e => setForm(f => ({...f, study_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Objective</label>
                <input type="text" value={form.objective} onChange={e => setForm(f => ({...f, objective: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Method</label>
                <input type="text" value={form.method} onChange={e => setForm(f => ({...f, method: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Lead Researcher</label>
                <input type="text" value={form.lead_researcher} onChange={e => setForm(f => ({...f, lead_researcher: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Field Days</label>
                <input type="text" value={form.field_days} onChange={e => setForm(f => ({...f, field_days: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
