...
interface MainRow {
  id: string;
    framework_name: string | number | boolean | string[] | null;
  framework_type: string | number | boolean | string[] | null;
  version: string | number | boolean | string[] | null;
  applicability: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  framework_id: string;
    control_id: string | number | boolean | string[] | null;
  ...
  test_frequency: string | number | boolean | string[] | null;
  last_tested: string | number | boolean | string[] | null;
  result: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function GrcPlatformPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "framework_name": "", "framework_type": "", "version": "", "applicability": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Assess GRC framework control coverage and identify compliance gaps');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/grc-platform');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/grc-platform?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'GRC Platform', secondary: 'Grc Control', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage grc platform data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No GRC Platform items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Framework Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Framework Type</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Version</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Applicability</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.framework_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.framework_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.version ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.applicability ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Grc Control</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total GRC Platform Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No grc control records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.control_id ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.control_name ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your grc platform data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add GRC Platform</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Framework Name</label>
                <input type="text" value={form.framework_name} onChange={e => setForm(f => ({...f, framework_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Framework Type</label>
                <input type="text" value={form.framework_type} onChange={e => setForm(f => ({...f, framework_type: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Applicability</label>
                <input type="text" value={form.applicability} onChange={e => setForm(f => ({...f, applicability: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
