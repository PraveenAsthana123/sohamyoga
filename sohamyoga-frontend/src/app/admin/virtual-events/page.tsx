...
interface MainRow {
  ...
  format: string | number | boolean | string[] | null;
  platform: string | number | boolean | string[] | null;
  event_date: string | number | boolean | string[] | null;
  start_time: string | number | boolean | string[] | null;
  duration_min: string | number | boolean | string[] | null;
  registered: string | number | boolean | string[] | null;
  attended: string | number | boolean | string[] | null;
  recording_url: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  event_id: string;
    session_name: string | number | boolean | string[] | null;
  speaker: string | number | boolean | string[] | null;
  duration_min: string | number | boolean | string[] | null;
  attendees: string | number | boolean | string[] | null;
  recording_url: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function VirtualEventsPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "event_name": "", "format": "", "platform": "", "event_date": "", "start_time": "", "duration_min": "", "registered": "", "attended": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze virtual event attendance and engagement metrics, suggest improvement strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/virtual-events');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/virtual-events?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Virtual & Hybrid Event Hub', secondary: 'Virtual Session', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">💻 Virtual & Hybrid Event Hub</h1>
          <p className="text-gray-500 text-sm mt-1">Manage virtual & hybrid event hub data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Virtual & Hybrid Event Hub items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Duration Min</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.format ?? '—')}</td>
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.start_time ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.duration_min ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Virtual Session</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Virtual & Hybrid Event Hub Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No virtual session records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Session Name</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.session_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.speaker ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.duration_min ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.attendees ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your virtual & hybrid event hub data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Virtual & Hybrid Event Hub</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.start_time} onChange={e => setForm(f => ({...f, start_time: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Duration Min</label>
                <input type="text" value={form.duration_min} onChange={e => setForm(f => ({...f, duration_min: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Registered</label>
                <input type="text" value={form.registered} onChange={e => setForm(f => ({...f, registered: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Attended</label>
                <input type="text" value={form.attended} onChange={e => setForm(f => ({...f, attended: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
