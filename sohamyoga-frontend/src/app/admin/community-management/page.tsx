...
interface MainRow {
  id: string;
    platform_name: string | number | boolean | string[] | null;
  community_name: string | number | boolean | string[] | null;
  member_count: string | number | boolean | string[] | null;
  moderators: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  author: string | number | boolean | string[] | null;
  reactions: string | number | boolean | string[] | null;
  comments: string | number | boolean | string[] | null;
  flagged: string | number | boolean | string[] | null;
...
export default function CommunityManagementPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "platform_name": "", "community_name": "", "member_count": "", "moderators": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze community engagement and suggest moderation and growth strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/community-management');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/community-management?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Community Management Hub', secondary: 'Community Post', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">👥 Community Management Hub</h1>
          <p className="text-gray-500 text-sm mt-1">Manage community management hub data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Community Management Hub items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Platform Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Community Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Member Count</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Moderators</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.platform_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.community_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.member_count ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.moderators ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Community Post</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Community Management Hub Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No community post records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Reactions</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.content ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.author ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.reactions ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.comments ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your community management hub data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Community Management Hub</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Platform Name</label>
                <input type="text" value={form.platform_name} onChange={e => setForm(f => ({...f, platform_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Community Name</label>
                <input type="text" value={form.community_name} onChange={e => setForm(f => ({...f, community_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Member Count</label>
                <input type="text" value={form.member_count} onChange={e => setForm(f => ({...f, member_count: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Moderators (comma-separated)</label>
                <input type="text" value={form.moderators} onChange={e => setForm(f => ({...f, moderators: e.target.value}))}
