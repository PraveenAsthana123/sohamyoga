...
interface MainRow {
  id: string;
    initiative_name: string | number | boolean | string[] | null;
  workstream: string | number | boolean | string[] | null;
  lead: string | number | boolean | string[] | null;
  members: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  initiative_id: string;
    artifact_name: string | number | boolean | string[] | null;
  artifact_type: string | number | boolean | string[] | null;
...
export default function AiCoePage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "initiative_name": "", "workstream": "", "lead": "", "members": "", "objective": "", "status": "", "start_date": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Assess AI CoE initiatives and recommend governance and capability building strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/ai-coe');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/ai-coe?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'AI Center of Excellence', secondary: 'Coe Artifact', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🏛️ AI Center of Excellence</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ai center of excellence data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No AI Center of Excellence items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Initiative Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Workstream</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.initiative_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.workstream ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.lead ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.members ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Coe Artifact</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total AI Center of Excellence Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No coe artifact records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Artifact Name</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.artifact_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.artifact_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.version ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your ai center of excellence data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add AI Center of Excellence</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Initiative Name</label>
                <input type="text" value={form.initiative_name} onChange={e => setForm(f => ({...f, initiative_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Workstream</label>
                <input type="text" value={form.workstream} onChange={e => setForm(f => ({...f, workstream: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Lead</label>
                <input type="text" value={form.lead} onChange={e => setForm(f => ({...f, lead: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Members (comma-separated)</label>
                <input type="text" value={form.members} onChange={e => setForm(f => ({...f, members: e.target.value}))}
