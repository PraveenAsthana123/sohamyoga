...
interface MainRow {
  id: string;
    template_name: string | number | boolean | string[] | null;
  tone: string | number | boolean | string[] | null;
  rating_range: string | number | boolean | string[] | null;
  category: string | number | boolean | string[] | null;
  template_text: string | number | boolean | string[] | null;
  use_count: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  template_id: string;
    variable_name: string | number | boolean | string[] | null;
...
export default function ReviewTemplatesPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "template_name": "", "tone": "", "rating_range": "", "category": "", "template_text": "", "use_count": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Generate personalized review response templates for different ratings and sentiments');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/review-templates?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Review Response Templates', secondary: 'Template Variable', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">📝 Review Response Templates</h1>
          <p className="text-gray-500 text-sm mt-1">Manage review response templates data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Review Response Templates items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Template Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tone</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Rating Range</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Category</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Template Text</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Use Count</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.template_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.tone ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.rating_range ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.category ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.template_text ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.use_count ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Template Variable</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Review Response Templates Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No template variable records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.variable_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.description ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.example ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your review response templates data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Review Response Templates</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Template Name</label>
                <input type="text" value={form.template_name} onChange={e => setForm(f => ({...f, template_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tone</label>
                <input type="text" value={form.tone} onChange={e => setForm(f => ({...f, tone: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Rating Range</label>
                <input type="text" value={form.rating_range} onChange={e => setForm(f => ({...f, rating_range: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Template Text</label>
                <input type="text" value={form.template_text} onChange={e => setForm(f => ({...f, template_text: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Use Count</label>
                <input type="text" value={form.use_count} onChange={e => setForm(f => ({...f, use_count: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
