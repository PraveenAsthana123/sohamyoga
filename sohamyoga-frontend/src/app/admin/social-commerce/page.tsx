...
interface MainRow {
  id: string;
    product_name: string | number | boolean | string[] | null;
  platform: string | number | boolean | string[] | null;
  listing_url: string | number | boolean | string[] | null;
  price: string | number | boolean | string[] | null;
  clicks: string | number | boolean | string[] | null;
  orders: string | number | boolean | string[] | null;
  revenue: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  product_id: string;
    platform: string | number | boolean | string[] | null;
  order_ref: string | number | boolean | string[] | null;
  amount: string | number | boolean | string[] | null;
  sold_at: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function SocialCommercePage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "product_name": "", "platform": "", "listing_url": "", "price": "", "clicks": "", "orders": "", "revenue": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze social commerce performance and suggest product listing optimizations');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/social-commerce?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Social Commerce', secondary: 'Social Sale', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage social commerce data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Social Commerce items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Listing Url</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.product_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.platform ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.listing_url ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.price ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.clicks ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.orders ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Social Sale</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Social Commerce Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No social sale records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sold At</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(c.order_ref ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.amount ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.sold_at ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your social commerce data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Social Commerce</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.listing_url} onChange={e => setForm(f => ({...f, listing_url: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.clicks} onChange={e => setForm(f => ({...f, clicks: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Orders</label>
                <input type="text" value={form.orders} onChange={e => setForm(f => ({...f, orders: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Revenue</label>
                <input type="text" value={form.revenue} onChange={e => setForm(f => ({...f, revenue: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
