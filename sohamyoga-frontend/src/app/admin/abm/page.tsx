...
interface MainRow {
  ...
  industry: string | number | boolean | string[] | null;
  tier: string | number | boolean | string[] | null;
  revenue_potential: string | number | boolean | string[] | null;
  primary_contact: string | number | boolean | string[] | null;
  email: string | number | boolean | string[] | null;
  phone: string | number | boolean | string[] | null;
  assigned_rep: string | number | boolean | string[] | null;
  engagement_score: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  account_id: string;
    touchpoint_type: string | number | boolean | string[] | null;
  channel: string | number | boolean | string[] | null;
  notes: string | number | boolean | string[] | null;
  outcome: string | number | boolean | string[] | null;
...
export default function AbmPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "company_name": "", "industry": "", "tier": "", "revenue_potential": "", "primary_contact": "", "email": "", "phone": "", "assigned_rep": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Prioritize ABM target accounts and suggest personalized engagement strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/abm');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/abm?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Account-Based Marketing', secondary: 'Abm Touchpoint', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🏢 Account-Based Marketing</h1>
          <p className="text-gray-500 text-sm mt-1">Manage account-based marketing data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Account-Based Marketing items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Revenue Potential</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.industry ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.tier ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.revenue_potential ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.primary_contact ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Abm Touchpoint</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Account-Based Marketing Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No abm touchpoint records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Touchpoint Type</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.touchpoint_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.channel ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.notes ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.outcome ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your account-based marketing data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Account-Based Marketing</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.tier} onChange={e => setForm(f => ({...f, tier: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Revenue Potential</label>
                <input type="number" value={form.revenue_potential} onChange={e => setForm(f => ({...f, revenue_potential: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Primary Contact</label>
                <input type="text" value={form.primary_contact} onChange={e => setForm(f => ({...f, primary_contact: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Assigned Rep</label>
                <input type="text" value={form.assigned_rep} onChange={e => setForm(f => ({...f, assigned_rep: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
