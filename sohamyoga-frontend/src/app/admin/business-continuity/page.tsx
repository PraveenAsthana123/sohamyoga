...
interface MainRow {
  id: string;
    plan_name: string | number | boolean | string[] | null;
  scope: string | number | boolean | string[] | null;
  rto_hours: string | number | boolean | string[] | null;
  rpo_hours: string | number | boolean | string[] | null;
  owner: string | number | boolean | string[] | null;
  last_tested: string | number | boolean | string[] | null;
  test_result: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  plan_id: string;
    scenario_name: string | number | boolean | string[] | null;
  probability: string | number | boolean | string[] | null;
  impact: string | number | boolean | string[] | null;
  response_steps: string | number | boolean | string[] | null;
  resources: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function BusinessContinuityPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "plan_name": "", "scope": "", "rto_hours": "", "rpo_hours": "", "owner": "", "last_tested": "", "test_result": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Review BCP plans for coverage gaps and recommend improvements to RTO/RPO targets');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/business-continuity');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/business-continuity?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Business Continuity Management', secondary: 'Bcp Scenario', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🔁 Business Continuity Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage business continuity management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Business Continuity Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Rto Hours</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Rpo Hours</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.plan_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.scope ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.rto_hours ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.rpo_hours ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.owner ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.last_tested ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Bcp Scenario</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Business Continuity Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No bcp scenario records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Scenario Name</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Response Steps</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.scenario_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.probability ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.impact ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.response_steps ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your business continuity management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Business Continuity Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.scope} onChange={e => setForm(f => ({...f, scope: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Rto Hours</label>
                <input type="text" value={form.rto_hours} onChange={e => setForm(f => ({...f, rto_hours: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Rpo Hours</label>
                <input type="text" value={form.rpo_hours} onChange={e => setForm(f => ({...f, rpo_hours: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" value={form.last_tested} onChange={e => setForm(f => ({...f, last_tested: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Test Result</label>
                <input type="text" value={form.test_result} onChange={e => setForm(f => ({...f, test_result: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
