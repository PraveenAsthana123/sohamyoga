...
interface MainRow {
  id: string;
    title: string | number | boolean | string[] | null;
  problem: string | number | boolean | string[] | null;
  date_occurred: string | number | boolean | string[] | null;
  reported_by: string | number | boolean | string[] | null;
  root_cause: string | number | boolean | string[] | null;
  corrective_action: string | number | boolean | string[] | null;
  preventive_action: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  closed_at: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  case_id: string;
    why_level: string | number | boolean | string[] | null;
  cause_text: string | number | boolean | string[] | null;
  created_at: string;
}

export default function RcaToolPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "title": "", "problem": "", "date_occurred": "", "reported_by": "", "root_cause": "", "corrective_action": "", "preventive_action": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Perform 5-Why root cause analysis and suggest corrective and preventive actions');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/rca-tool');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/rca-tool?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Root Cause Analysis', secondary: 'Rca Cause', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage root cause analysis data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Root Cause Analysis items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date Occurred</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.title ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.problem ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.date_occurred ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.reported_by ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.root_cause ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.corrective_action ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Rca Cause</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Root Cause Analysis Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No rca cause records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Why Level</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cause Text</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.why_level ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.cause_text ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your root cause analysis data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Root Cause Analysis</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.problem} onChange={e => setForm(f => ({...f, problem: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Date Occurred</label>
              ...
              <div>
                ...
                <input type="text" value={form.reported_by} onChange={e => setForm(f => ({...f, reported_by: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Root Cause</label>
                <input type="text" value={form.root_cause} onChange={e => setForm(f => ({...f, root_cause: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Corrective Action</label>
                <input type="text" value={form.corrective_action} onChange={e => setForm(f => ({...f, corrective_action: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Preventive Action</label>
                <input type="text" value={form.preventive_action} onChange={e => setForm(f => ({...f, preventive_action: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
