...
interface MainRow {
  id: string;
    report_title: string | number | boolean | string[] | null;
  publisher: string | number | boolean | string[] | null;
  topic: string | number | boolean | string[] | null;
  industry: string | number | boolean | string[] | null;
  coverage_period: string | number | boolean | string[] | null;
  purchase_date: string | number | boolean | string[] | null;
  cost: string | number | boolean | string[] | null;
  pages: string | number | boolean | string[] | null;
  access_url: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  report_id: string;
    insight: string | number | boolean | string[] | null;
  relevance_score: string | number | boolean | string[] | null;
  tags: string | number | boolean | string[] | null;
  extracted_at: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function SyndicatedResearchPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "report_title": "", "publisher": "", "topic": "", "industry": "", "coverage_period": "", "purchase_date": "", "cost": "", "pages": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Extract key insights from syndicated research and relate them to business strategy');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/syndicated-research');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/syndicated-research?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Syndicated Research', secondary: 'Research Insight', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">📊 Syndicated Research</h1>
          <p className="text-gray-500 text-sm mt-1">Manage syndicated research data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Syndicated Research items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Report Title</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Coverage Period</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.report_title ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.publisher ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.topic ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.industry ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.coverage_period ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.purchase_date ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Research Insight</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Syndicated Research Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No research insight records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Insight</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Relevance Score</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tags</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Extracted At</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.insight ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.relevance_score ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.tags ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.extracted_at ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your syndicated research data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Syndicated Research</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Report Title</label>
                <input type="text" value={form.report_title} onChange={e => setForm(f => ({...f, report_title: e.target.value}))}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Coverage Period</label>
                <input type="text" value={form.coverage_period} onChange={e => setForm(f => ({...f, coverage_period: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
