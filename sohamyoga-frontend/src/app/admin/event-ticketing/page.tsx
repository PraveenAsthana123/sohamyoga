...
interface MainRow {
  id: string;
    event_name: string | number | boolean | string[] | null;
  event_date: string | number | boolean | string[] | null;
  venue: string | number | boolean | string[] | null;
  total_capacity: string | number | boolean | string[] | null;
  tickets_sold: string | number | boolean | string[] | null;
  ticket_price: string | number | boolean | string[] | null;
  early_bird_price: string | number | boolean | string[] | null;
  early_bird_until: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  config_id: string;
    attendee_name: string | number | boolean | string[] | null;
  email: string | number | boolean | string[] | null;
  ticket_type: string | number | boolean | string[] | null;
  quantity: string | number | boolean | string[] | null;
  amount_paid: string | number | boolean | string[] | null;
  registration_date: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function EventTicketingPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "event_name": "", "event_date": "", "venue": "", "total_capacity": "", "tickets_sold": "", "ticket_price": "", "early_bird_price": "", "early_bird_until": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze ticket sales performance and suggest pricing and promotion strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/event-ticketing');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/event-ticketing?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Event Registration & Ticketing', secondary: 'Ticket Registration', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🎟️ Event Registration & Ticketing</h1>
          <p className="text-gray-500 text-sm mt-1">Manage event registration & ticketing data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Event Registration & Ticketing items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Total Capacity</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.event_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.event_date ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.venue ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.total_capacity ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.tickets_sold ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.ticket_price ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Ticket Registration</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Event Registration & Ticketing Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No ticket registration records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Attendee Name</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.attendee_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.email ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.ticket_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.quantity ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your event registration & ticketing data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Event Registration & Ticketing</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Event Name</label>
                <input type="text" value={form.event_name} onChange={e => setForm(f => ({...f, event_name: e.target.value}))}
              ...
              <div>
                ...
                <input type="text" value={form.total_capacity} onChange={e => setForm(f => ({...f, total_capacity: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tickets Sold</label>
                <input type="text" value={form.tickets_sold} onChange={e => setForm(f => ({...f, tickets_sold: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Ticket Price</label>
                <input type="number" value={form.ticket_price} onChange={e => setForm(f => ({...f, ticket_price: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Early Bird Price</label>
                <input type="number" value={form.early_bird_price} onChange={e => setForm(f => ({...f, early_bird_price: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Early Bird Until</label>
                <input type="date" value={form.early_bird_until} onChange={e => setForm(f => ({...f, early_bird_until: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
