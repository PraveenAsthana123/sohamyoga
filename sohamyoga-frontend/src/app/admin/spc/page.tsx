...
interface MainRow {
  ...
  lsl: string | number | boolean | string[] | null;
  target: string | number | boolean | string[] | null;
  sample_size: string | number | boolean | string[] | null;
  frequency: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  subgroup: string | number | boolean | string[] | null;
  out_of_control: string | number | boolean | string[] | null;
...
export default function SpcPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "chart_name": "", "process_name": "", "metric": "", "usl": "", "lsl": "", "target": "", "sample_size": "", "frequency": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze SPC data for out-of-control signals and process capability');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/spc?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Statistical Process Control', secondary: 'Spc Data Point', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage statistical process control data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Statistical Process Control items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Lsl</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.metric ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.usl ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.lsl ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.target ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Spc Data Point</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Statistical Process Control Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No spc data point records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Value</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Subgroup</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Out Of Control</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.value ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.subgroup ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.out_of_control ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your statistical process control data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Statistical Process Control</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.metric} onChange={e => setForm(f => ({...f, metric: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Lsl</label>
                <input type="number" value={form.lsl} onChange={e => setForm(f => ({...f, lsl: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.sample_size} onChange={e => setForm(f => ({...f, sample_size: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Frequency</label>
                <input type="text" value={form.frequency} onChange={e => setForm(f => ({...f, frequency: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
