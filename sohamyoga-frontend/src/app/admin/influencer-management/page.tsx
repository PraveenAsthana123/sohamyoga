...
interface MainRow {
  ...
  handle: string | number | boolean | string[] | null;
  platform: string | number | boolean | string[] | null;
  follower_count: string | number | boolean | string[] | null;
  engagement_rate: string | number | boolean | string[] | null;
  niche: string | number | boolean | string[] | null;
  email: string | number | boolean | string[] | null;
  fee_per_post: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  deliverables: string | number | boolean | string[] | null;
  due_date: string | number | boolean | string[] | null;
  fee_paid: string | number | boolean | string[] | null;
  impressions: string | number | boolean | string[] | null;
  conversions: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function InfluencerManagementPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "name": "", "handle": "", "platform": "", "follower_count": "", "engagement_rate": "", "niche": "", "email": "", "fee_per_post": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Evaluate influencer performance and suggest collaboration strategies for maximum ROI');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/influencer-management');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/influencer-management?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Influencer Management', secondary: 'Influencer Campaign', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage influencer management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Influencer Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Follower Count</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.handle ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.platform ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.follower_count ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.engagement_rate ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.niche ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Influencer Campaign</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Influencer Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No influencer campaign records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Campaign Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Deliverables</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Due Date</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Fee Paid</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.campaign_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.deliverables ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.due_date ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.fee_paid ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your influencer management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Influencer Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Follower Count</label>
                <input type="text" value={form.follower_count} onChange={e => setForm(f => ({...f, follower_count: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Engagement Rate</label>
                <input type="number" value={form.engagement_rate} onChange={e => setForm(f => ({...f, engagement_rate: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Fee Per Post</label>
                <input type="number" value={form.fee_per_post} onChange={e => setForm(f => ({...f, fee_per_post: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
