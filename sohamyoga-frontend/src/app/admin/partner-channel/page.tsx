...
interface MainRow {
  ...
  partner_type: string | number | boolean | string[] | null;
  ...
  territory: string | number | boolean | string[] | null;
  tier: string | number | boolean | string[] | null;
  revenue_ytd: string | number | boolean | string[] | null;
  deal_count: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  partner_id: string;
    deal_name: string | number | boolean | string[] | null;
  client_name: string | number | boolean | string[] | null;
  value: string | number | boolean | string[] | null;
  stage: string | number | boolean | string[] | null;
  close_date: string | number | boolean | string[] | null;
  commission_pct: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function PartnerChannelPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "partner_name": "", "partner_type": "", "contact_name": "", "email": "", "phone": "", "territory": "", "tier": "", "revenue_ytd": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze channel partner performance and recommend enablement and incentive strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/partner-channel');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/partner-channel?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Partner & Channel Management', secondary: 'Partner Deal', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🤝 Partner & Channel Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage partner & channel management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Partner & Channel Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.partner_type ?? '—')}</td>
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.territory ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Partner Deal</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Partner & Channel Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No partner deal records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.deal_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.client_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.value ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.stage ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your partner & channel management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Partner & Channel Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.partner_type} onChange={e => setForm(f => ({...f, partner_type: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Revenue Ytd</label>
                <input type="text" value={form.revenue_ytd} onChange={e => setForm(f => ({...f, revenue_ytd: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
