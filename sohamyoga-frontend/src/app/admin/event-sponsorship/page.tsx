...
interface MainRow {
  id: string;
    company_name: string | number | boolean | string[] | null;
  contact_name: string | number | boolean | string[] | null;
  email: string | number | boolean | string[] | null;
  phone: string | number | boolean | string[] | null;
  sponsor_tier: string | number | boolean | string[] | null;
  commitment_amount: string | number | boolean | string[] | null;
  paid_amount: string | number | boolean | string[] | null;
  benefits: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  activation_type: string | number | boolean | string[] | null;
  logo_placement: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function EventSponsorshipPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "company_name": "", "contact_name": "", "email": "", "phone": "", "sponsor_tier": "", "commitment_amount": "", "paid_amount": "", "benefits": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze sponsorship ROI and suggest tiering and benefit packages');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/event-sponsorship');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/event-sponsorship?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Event Sponsorship Management', secondary: 'Sponsorship Event', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🤝 Event Sponsorship Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage event sponsorship management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Event Sponsorship Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Sponsor Tier</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Commitment Amount</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.company_name ?? '—')}</td>
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.phone ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.sponsor_tier ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.commitment_amount ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Sponsorship Event</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Event Sponsorship Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No sponsorship event records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Activation Type</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Logo Placement</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.event_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.event_date ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.activation_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.logo_placement ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your event sponsorship management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Event Sponsorship Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Sponsor Tier</label>
                <input type="text" value={form.sponsor_tier} onChange={e => setForm(f => ({...f, sponsor_tier: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Commitment Amount</label>
                <input type="text" value={form.commitment_amount} onChange={e => setForm(f => ({...f, commitment_amount: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Paid Amount</label>
                <input type="text" value={form.paid_amount} onChange={e => setForm(f => ({...f, paid_amount: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
