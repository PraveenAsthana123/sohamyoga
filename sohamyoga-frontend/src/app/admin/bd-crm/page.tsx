...
interface MainRow {
  ...
  linkedin: string | number | boolean | string[] | null;
  industry: string | number | boolean | string[] | null;
  company_size: string | number | boolean | string[] | null;
  source: string | number | boolean | string[] | null;
  last_contact: string | number | boolean | string[] | null;
  next_action: string | number | boolean | string[] | null;
  bd_stage: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  contact_id: string;
    activity_type: string | number | boolean | string[] | null;
  notes: string | number | boolean | string[] | null;
  outcome: string | number | boolean | string[] | null;
  next_step: string | number | boolean | string[] | null;
  activity_date: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function BdCrmPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "company_name": "", "contact_name": "", "title": "", "email": "", "phone": "", "linkedin": "", "industry": "", "company_size": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze BD pipeline and recommend prioritization and outreach strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/bd-crm');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/bd-crm?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Business Development CRM', secondary: 'Bd Activity', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">💼 Business Development CRM</h1>
          <p className="text-gray-500 text-sm mt-1">Manage business development crm data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Business Development CRM items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.linkedin ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Bd Activity</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Business Development CRM Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No bd activity records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.activity_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.notes ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.outcome ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.next_step ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your business development crm data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Business Development CRM</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.company_size} onChange={e => setForm(f => ({...f, company_size: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
