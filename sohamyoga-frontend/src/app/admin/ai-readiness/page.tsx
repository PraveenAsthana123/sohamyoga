...
interface MainRow {
  ...
  data_maturity_score: string | number | boolean | string[] | null;
  infra_score: string | number | boolean | string[] | null;
  talent_score: string | number | boolean | string[] | null;
  process_score: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  recommendation: string | number | boolean | string[] | null;
...
export default function AiReadinessPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "organization": "", "assessment_date": "", "assessor": "", "data_maturity_score": "", "infra_score": "", "talent_score": "", "process_score": "", "governance_score": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze AI readiness scores and provide a detailed roadmap for AI adoption');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/ai-readiness?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'AI Readiness Assessment', secondary: 'Readiness Finding', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage ai readiness assessment data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No AI Readiness Assessment items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Data Maturity Score</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Infra Score</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Talent Score</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.assessor ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.data_maturity_score ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.infra_score ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.talent_score ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Readiness Finding</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total AI Readiness Assessment Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No readiness finding records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(c.recommendation ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.priority ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your ai readiness assessment data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add AI Readiness Assessment</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.assessor} onChange={e => setForm(f => ({...f, assessor: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Data Maturity Score</label>
                <input type="text" value={form.data_maturity_score} onChange={e => setForm(f => ({...f, data_maturity_score: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Infra Score</label>
                <input type="text" value={form.infra_score} onChange={e => setForm(f => ({...f, infra_score: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Talent Score</label>
                <input type="text" value={form.talent_score} onChange={e => setForm(f => ({...f, talent_score: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Process Score</label>
                <input type="text" value={form.process_score} onChange={e => setForm(f => ({...f, process_score: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
