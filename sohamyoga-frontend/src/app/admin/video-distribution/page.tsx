...
interface MainRow {
  ...
  duration: string | number | boolean | string[] | null;
  channels: string | number | boolean | string[] | null;
  views_total: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  publish_url: string | number | boolean | string[] | null;
  scheduled_at: string | number | boolean | string[] | null;
...
export default function VideoDistributionPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "title": "", "format": "", "duration": "", "channels": "", "views_total": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze video distribution performance across channels and recommend optimization strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/video-distribution');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/video-distribution?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Video Distribution Hub', secondary: 'Distribution Job', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">📺 Video Distribution Hub</h1>
          <p className="text-gray-500 text-sm mt-1">Manage video distribution hub data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Video Distribution Hub items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Views Total</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.duration ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.channels ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.views_total ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Distribution Job</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Video Distribution Hub Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No distribution job records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Channel</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Publish Url</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Scheduled At</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Published At</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(c.publish_url ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.scheduled_at ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.published_at ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your video distribution hub data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Video Distribution Hub</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Channels (comma-separated)</label>
                <input type="text" value={form.channels} onChange={e => setForm(f => ({...f, channels: e.target.value}))}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Views Total</label>
                <input type="text" value={form.views_total} onChange={e => setForm(f => ({...f, views_total: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
