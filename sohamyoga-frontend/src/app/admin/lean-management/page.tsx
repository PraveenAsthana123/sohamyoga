...
interface MainRow {
  ...
  waste_type: string | number | boolean | string[] | null;
  lead_time_before: string | number | boolean | string[] | null;
  lead_time_after: string | number | boolean | string[] | null;
  savings_annual: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  project_id: string;
    action_item: string | number | boolean | string[] | null;
  assigned_to: string | number | boolean | string[] | null;
  due_date: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}

export default function LeanManagementPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "project_name": "", "area": "", "waste_type": "", "lead_time_before": "", "lead_time_after": "", "savings_annual": "", "owner": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Identify lean waste elimination opportunities and suggest kaizen improvements');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/lean-management');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/lean-management?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Lean Management', secondary: 'Lean Action', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🔄 Lean Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage lean management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Lean Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Lead Time Before</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Lead Time After</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Savings Annual</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.waste_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.lead_time_before ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.lead_time_after ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.savings_annual ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Lean Action</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Lean Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No lean action records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Action Item</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.action_item ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.assigned_to ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your lean management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Lean Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Waste Type (comma-separated)</label>
                <input type="text" value={form.waste_type} onChange={e => setForm(f => ({...f, waste_type: e.target.value}))}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Lead Time Before</label>
                <input type="number" value={form.lead_time_before} onChange={e => setForm(f => ({...f, lead_time_before: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Lead Time After</label>
                <input type="number" value={form.lead_time_after} onChange={e => setForm(f => ({...f, lead_time_after: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Savings Annual</label>
                <input type="number" value={form.savings_annual} onChange={e => setForm(f => ({...f, savings_annual: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
