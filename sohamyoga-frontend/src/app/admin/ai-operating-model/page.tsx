...
interface MainRow {
  id: string;
    layer_name: string | number | boolean | string[] | null;
  layer_type: string | number | boolean | string[] | null;
  description: string | number | boolean | string[] | null;
  owner: string | number | boolean | string[] | null;
  tools: string | number | boolean | string[] | null;
  maturity_level: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  layer_id: string;
    kpi_name: string | number | boolean | string[] | null;
  current_value: string | number | boolean | string[] | null;
  target_value: string | number | boolean | string[] | null;
  unit: string | number | boolean | string[] | null;
  last_updated: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function AiOperatingModelPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "layer_name": "", "layer_type": "", "description": "", "owner": "", "tools": "", "maturity_level": "", "status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Review AI operating model layers and recommend improvements for scalability');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/ai-operating-model');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/ai-operating-model?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'AI Operating Model', secondary: 'Operating Kpi', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">⚙️ AI Operating Model</h1>
          <p className="text-gray-500 text-sm mt-1">Manage ai operating model data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No AI Operating Model items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Layer Name</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Layer Type</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Description</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Owner</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tools</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Maturity Level</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.layer_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.layer_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.description ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.owner ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.tools ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.maturity_level ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Operating Kpi</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total AI Operating Model Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No operating kpi records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.kpi_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.current_value ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.target_value ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.unit ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your ai operating model data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add AI Operating Model</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Layer Name</label>
                <input type="text" value={form.layer_name} onChange={e => setForm(f => ({...f, layer_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Layer Type</label>
                <input type="text" value={form.layer_type} onChange={e => setForm(f => ({...f, layer_type: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tools (comma-separated)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Maturity Level</label>
                <input type="text" value={form.maturity_level} onChange={e => setForm(f => ({...f, maturity_level: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
