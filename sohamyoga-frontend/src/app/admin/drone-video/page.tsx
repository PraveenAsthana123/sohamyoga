...
interface MainRow {
  ...
  flight_date: string | number | boolean | string[] | null;
  pilot_name: string | number | boolean | string[] | null;
  equipment: string | number | boolean | string[] | null;
  footage_duration_min: string | number | boolean | string[] | null;
  edit_status: string | number | boolean | string[] | null;
  delivery_date: string | number | boolean | string[] | null;
  amount: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  project_id: string;
    file_name: string | number | boolean | string[] | null;
  resolution: string | number | boolean | string[] | null;
  duration_sec: string | number | boolean | string[] | null;
  storage_url: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function DroneVideoPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "project_name": "", "client_name": "", "location": "", "flight_date": "", "pilot_name": "", "equipment": "", "footage_duration_min": "", "edit_status": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Review drone video projects and suggest post-production workflow improvements');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/drone-video');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/drone-video?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Drone Video Management', secondary: 'Drone Asset', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🚁 Drone Video Management</h1>
          <p className="text-gray-500 text-sm mt-1">Manage drone video management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Drone Video Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Pilot Name</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.location ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.flight_date ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.pilot_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.equipment ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Drone Asset</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Drone Video Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No drone asset records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Duration Sec</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Storage Url</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.file_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.resolution ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.duration_sec ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.storage_url ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your drone video management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Drone Video Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Flight Date</label>
                <input type="date" value={form.flight_date} onChange={e => setForm(f => ({...f, flight_date: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Pilot Name</label>
                <input type="text" value={form.pilot_name} onChange={e => setForm(f => ({...f, pilot_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Footage Duration Min</label>
                <input type="number" value={form.footage_duration_min} onChange={e => setForm(f => ({...f, footage_duration_min: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Edit Status</label>
                <input type="text" value={form.edit_status} onChange={e => setForm(f => ({...f, edit_status: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
