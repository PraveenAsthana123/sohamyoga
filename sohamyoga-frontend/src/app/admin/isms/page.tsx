...
interface MainRow {
  id: string;
    iso_clause: string | number | boolean | string[] | null;
  control_name: string | number | boolean | string[] | null;
  ...
  evidence: string | number | boolean | string[] | null;
  last_reviewed: string | number | boolean | string[] | null;
  risk_treatment: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  control_id: string;
    asset: string | number | boolean | string[] | null;
  threat: string | number | boolean | string[] | null;
  vulnerability: string | number | boolean | string[] | null;
  likelihood: string | number | boolean | string[] | null;
  impact: string | number | boolean | string[] | null;
  risk_treatment: string | number | boolean | string[] | null;
...
export default function IsmsPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "iso_clause": "", "control_name": "", "description": "", "implementation_status": "", "owner": "", "evidence": "", "last_reviewed": "", "risk_treatment": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Assess ISO 27001 control implementation and identify information security risks');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/isms');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/isms?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'ISMS / ISO 27001', secondary: 'Isms Risk', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage isms / iso 27001 data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No ISMS / ISO 27001 items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Iso Clause</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.iso_clause ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.control_name ?? '—')}</td>
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(item.evidence ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Isms Risk</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total ISMS / ISO 27001 Items</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-green-700">{items.filter(i => String(i.implementation_status) === 'implemented').length}</div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No isms risk records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.asset ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.threat ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.vulnerability ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.likelihood ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your isms / iso 27001 data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add ISMS / ISO 27001</h3>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <div className="space-y-3">
                            <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Iso Clause</label>
                <input type="text" value={form.iso_clause} onChange={e => setForm(f => ({...f, iso_clause: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Control Name</label>
                <input type="text" value={form.control_name} onChange={e => setForm(f => ({...f, control_name: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Last Reviewed</label>
                <input type="date" value={form.last_reviewed} onChange={e => setForm(f => ({...f, last_reviewed: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Risk Treatment</label>
                <input type="text" value={form.risk_treatment} onChange={e => setForm(f => ({...f, risk_treatment: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
