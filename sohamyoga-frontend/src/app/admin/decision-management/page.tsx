...
interface MainRow {
  id: string;
    rule_name: string | number | boolean | string[] | null;
  category: string | number | boolean | string[] | null;
  condition_description: string | number | boolean | string[] | null;
  action_description: string | number | boolean | string[] | null;
  priority: string | number | boolean | string[] | null;
  active: string | number | boolean | string[] | null;
  created_by: string | number | boolean | string[] | null;
...
interface ChildRow {
  id: string;
  rule_id: string;
    input_data: string | number | boolean | string[] | null;
  output_result: string | number | boolean | string[] | null;
...
export default function DecisionManagementPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "rule_name": "", "category": "", "condition_description": "", "action_description": "", "priority": "", "active": "", "created_by": "", "last_tested": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Review decision rules for conflicts, gaps, and optimization opportunities');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/decision-management');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/decision-management?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Decision Management', secondary: 'Decision Log', ai: 'AI Assistant' };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage decision management data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Decision Management items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Condition Description</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Action Description</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.rule_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.category ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.condition_description ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.action_description ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.priority ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.active ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Decision Log</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Decision Management Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No decision log records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Input Data</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Output Result</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.input_data ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.output_result ?? '—')}</td>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your decision management data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Decision Management</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Condition Description</label>
                <input type="text" value={form.condition_description} onChange={e => setForm(f => ({...f, condition_description: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Action Description</label>
                <input type="text" value={form.action_description} onChange={e => setForm(f => ({...f, action_description: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.active === 'true'} onChange={e => setForm(f => ({...f, active: String(e.target.checked)}))}/> Active
              </label>
              <div>
