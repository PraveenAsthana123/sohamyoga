...
interface MainRow {
  id: string;
    campaign_name: string | number | boolean | string[] | null;
  channel: string | number | boolean | string[] | null;
  tactic: string | number | boolean | string[] | null;
  budget: string | number | boolean | string[] | null;
  impressions: string | number | boolean | string[] | null;
  leads_generated: string | number | boolean | string[] | null;
  mqls: string | number | boolean | string[] | null;
  sqls: string | number | boolean | string[] | null;
  cost_per_lead: string | number | boolean | string[] | null;
  status: string | number | boolean | string[] | null;
  start_date: string | number | boolean | string[] | null;
  end_date: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  downloads: string | number | boolean | string[] | null;
  created_at: string;
}
...
export default function DemandGenerationPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "campaign_name": "", "channel": "", "tactic": "", "budget": "", "impressions": "", "leads_generated": "", "mqls": "", "sqls": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Analyze demand generation campaigns and recommend channel optimization strategies');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/demand-generation');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/demand-generation?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Demand Generation Hub', secondary: 'Demand Asset', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">📣 Demand Generation Hub</h1>
          <p className="text-gray-500 text-sm mt-1">Manage demand generation hub data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Demand Generation Hub items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tactic</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Budget</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Impressions</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Leads Generated</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.campaign_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.channel ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.tactic ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.budget ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.impressions ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.leads_generated ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Demand Asset</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Demand Generation Hub Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No demand asset records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(c.asset_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.asset_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.downloads ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your demand generation hub data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Demand Generation Hub</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tactic</label>
                <input type="text" value={form.tactic} onChange={e => setForm(f => ({...f, tactic: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.impressions} onChange={e => setForm(f => ({...f, impressions: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Leads Generated</label>
                <input type="text" value={form.leads_generated} onChange={e => setForm(f => ({...f, leads_generated: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Mqls</label>
                <input type="text" value={form.mqls} onChange={e => setForm(f => ({...f, mqls: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Sqls</label>
                <input type="text" value={form.sqls} onChange={e => setForm(f => ({...f, sqls: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
            </div>
