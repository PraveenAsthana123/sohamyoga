...
interface MainRow {
  id: string;
    asset_name: string | number | boolean | string[] | null;
  asset_type: string | number | boolean | string[] | null;
  data_types: string | number | boolean | string[] | null;
  data_subjects: string | number | boolean | string[] | null;
  processing_purpose: string | number | boolean | string[] | null;
  legal_basis: string | number | boolean | string[] | null;
  risk_level: string | number | boolean | string[] | null;
  dpia_required: string | number | boolean | string[] | null;
...
interface ChildRow {
  ...
  regulation: string | number | boolean | string[] | null;
...
export default function PrivacyRiskPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    "asset_name": "", "asset_type": "", "data_types": "", "data_subjects": "", "processing_purpose": "", "legal_basis": "", "risk_level": "", "dpia_required": ""
  });
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState('');
  const [aiPrompt, setAiPrompt] = useState('Review privacy assessments and identify GDPR/PIPEDA compliance gaps');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/admin/privacy-risk');
  ...
  const handleDelete = async (id: string) => {
    ...
    await fetch(`/api/admin/privacy-risk?id=${id}`, { method: 'DELETE' });
  ...
  const TAB_LABELS = { main: 'Privacy Risk Assessment', secondary: 'Privacy Finding', ai: 'AI Assistant' };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">🔐 Privacy Risk Assessment</h1>
          <p className="text-gray-500 text-sm mt-1">Manage privacy risk assessment data and analytics</p>
        </div>
      ...
      {tab === 'main' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">No Privacy Risk Assessment items yet. Click &quot;+ Add New&quot; to get started.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Data Subjects</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Processing Purpose</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 text-gray-900">{String(item.asset_name ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.asset_type ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.data_types ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.data_subjects ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.processing_purpose ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(item.legal_basis ?? '—')}</td>
      ...
      {tab === 'secondary' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Privacy Finding</h2>
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-2xl font-bold text-blue-700">{items.length}</div>
              <div className="text-sm text-blue-600">Total Privacy Risk Assessment Items</div>
            </div>
          ...
          {children.length === 0 ? (
            <div className="text-center text-gray-400 py-8">No privacy finding records yet.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {children.slice(0, 50).map(c => (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{String(c.regulation ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.severity ?? '—')}</td>
                    <td className="px-4 py-3 text-gray-900">{String(c.remediation ?? '—')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Ask AI about your privacy risk assessment data..."
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Add Privacy Risk Assessment</h3>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="text" value={form.asset_type} onChange={e => setForm(f => ({...f, asset_type: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Data Types (comma-separated)</label>
                <input type="text" value={form.data_types} onChange={e => setForm(f => ({...f, data_types: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="item1, item2"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Data Subjects</label>
                <input type="text" value={form.data_subjects} onChange={e => setForm(f => ({...f, data_subjects: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Processing Purpose</label>
                <input type="text" value={form.processing_purpose} onChange={e => setForm(f => ({...f, processing_purpose: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Legal Basis</label>
                <input type="text" value={form.legal_basis} onChange={e => setForm(f => ({...f, legal_basis: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Risk Level</label>
                <input type="text" value={form.risk_level} onChange={e => setForm(f => ({...f, risk_level: e.target.value}))}
                  className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
              </div>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.dpia_required === 'true'} onChange={e => setForm(f => ({...f, dpia_required: String(e.target.checked)}))}/> Dpia Required
              </label>
            </div>
