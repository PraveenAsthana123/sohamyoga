...
type RequestRow = {
  ...
  channels: string[]; model_name: string | null;
};
  ...
  try {
    ...
    const claimed = await client.query<RequestRow>(
      `SELECT id, tenant_id, title, industry, objective, audience, offer_text,
              call_to_action, asset_types, channels, model_name
...
Do not invent prices, certifications, medical outcomes, testimonials, or guarantees.`,
    ...
    await transactionResult(request, content);
  } catch (error) {
...
async function transactionResult(request: RequestRow, content: Record<string, unknown>): Promise<void> {
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    for (const assetType of request.asset_types) {
      ...
      await client.query(
        `INSERT INTO generated_marketing_asset
         (tenant_id, request_id, asset_type, status, text_content, metadata)
         VALUES ($1,$2,$3,'review_required',$4,$5)`,
        [request.tenant_id, request.id, assetType, text, JSON.stringify({ generatedBy: 'ollama', model: request.model_name, channels: request.channels })],
      );
    }
    await client.query(
    ...
    await client.query(
      ...
      [request.tenant_id, request.id, JSON.stringify({ assetTypes: request.asset_types, channels: request.channels })],
    );
    await client.query('COMMIT');
  } catch (error) {
    await client.query('ROLLBACK');
