...
const SUGGESTIONS = [
  ...
  'How does the AI pose coach work?',
  'Tell me about membership plans',
];

const GREETING =
  "Hi! I'm the SohamYoga AI assistant, powered by a local AI model. Ask me about classes, pose coaching, membership plans, or how to get started on your wellness journey.";
...
export default function AiAssistant() {
  ...
  const loadModels = useCallback(async () => {
    try {
      ...
      const data = (await res.json()) as { models?: Array<string | { name: string; enabled?: boolean }>; defaultModel?: string };
      ...
      if (models.length) {
        ...
        if (!selectedModel) {
          setSelectedModel(data.defaultModel || models[0]);
        }
      }
    } catch {
      // Ignore model discovery failures; the assistant will still work with the default model.
    }
  }, [selectedModel]);

  const checkHealth = useCallback(async (model?: string) => {
    try {
      const suffix = model ? `?model=${encodeURIComponent(model)}` : '';
      const res = await fetch(`/api/ai/health${suffix}`, { cache: 'no-store' });
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      if (healthStatus !== 'online') {
        ...
        setMessages([...nextMessages, { role: 'assistant', content: 'The local AI service is currently unavailable. Please check that Ollama is running and the selected model is installed.' }]);
        setInput('');
        return;
      }
    ...
    [healthStatus, messages, selectedModel, streaming],
  );
        ...
        <div className="fixed bottom-24 left-6 z-40 flex h-[32rem] w-[22rem] max-w-[calc(100vw-3rem)] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
          ...
          <div className="flex items-center gap-3 bg-primary-700 px-4 py-3 text-white">
            ...
            <div className="leading-tight">
              <p className="text-sm font-semibold">SohamYoga AI</p>
          ...
          <form onSubmit={onSubmit} className="flex items-center gap-2 border-t border-gray-200 bg-white px-3 py-3">
            <select
              ...
              className="rounded-full border border-gray-300 bg-white px-3 py-2 text-xs text-gray-700 outline-none"
            >
