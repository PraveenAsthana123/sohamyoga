...
INSERT INTO job_registry (name, schedule, description, module, enabled, timeout_ms) VALUES
  ...
  ('voice-call-dispatch', '*/5 * * * *', 'Every 5 minutes — Voice AI module''s Automatic Process. Sweeps voice_call rows whose scheduled_at has passed while still "scheduled" and relabels them "blocked" with the real reason (no telephony provider connected, or connected with no PSTN dispatch client built yet). Does not and cannot place real calls — no such client exists in this app.', 'VoiceCallDispatchJob', TRUE, 60000),
  ('topic-intelligence', '0 6 * * *', 'Daily 06:00 UTC — Topic Intelligence Engine. Mines real competitor entries, real lead inquiry messages, and real measured hook performance for recurring keywords, upserting topic_signal rows. Writes zero rows on a quiet day rather than fabricating a trend.', 'TopicIntelligenceJob', TRUE, 60000)
